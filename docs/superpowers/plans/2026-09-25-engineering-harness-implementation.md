...
- [ ] **Step 1: Record the tree and pin development dependencies.** Run `git status --short`, `git rev-parse HEAD`, `node --version`, `npm --version`; this review started at `0225fc8`, and execution must record any changed baseline before edits. Do not stage `.codegraph/`. Use `npm install --save-dev --save-exact eslint@10 @eslint/js@10 globals@16 prettier@3.9.9 typescript@5.9.3 @types/node@22`. **Deviation from the first plan revision, with the measurement behind it:** the pinned ESLint was 9, but ESLint 10.11.0 adds two rules to the recommended set — `no-useless-assignment` (12 findings) and `preserve-caught-error` (3) — both real signals, both already triaged in spec §6.2. The same tree yields 28 errors / 15 files under 9.39.5 and 43 / 22 under 10.11.0, so pinning 9 would be one major version of deliberately missing rules. Re-measure the baseline after install and use that number in the task note. Record resolved versions with `npm ls --depth=0` and include the lockfile. If a pinned package cannot be installed, resolve that package's actual compatible patch before changing the design; do not leave an unlocked range.
...
- [ ] **Step 1: Replace the starter config.** Use the flat config below; the `test/smoke` scripts remain in scope, while records and fixtures do not:
  ...
  Add `"lint": "eslint ."` to `package.json`. Run it and save the rule/file counts in the task note; this is the required red baseline (measured 2026-09-25 on ESLint 10.11.0: **43 errors across 22 of 58 files** — `no-unused-vars` 20, `no-useless-assignment` 12, `no-control-regex` 6, `preserve-caught-error` 3, `no-misleading-character-class` 2).
...
**Files:** Create `tsconfig.json`; modify `package.json`; add `// @ts-check` to the twelve measured files below; add/extend `test/repo-hygiene.test.js`; modify `CHANGELOG.md` if source behavior is corrected.

**Interfaces:** `npm run types` runs `tsc --noEmit` over explicitly opted-in JS; a source-file list test guards the markers **and** the agreement between the marked set and `tsconfig.files`. It does not claim every `lib/` file is clean.

**Measured opt-in set (2026-09-25, TypeScript 5.9.3, `checkJs:false`, `skipLibCheck:true`, one file at a time over 28 candidates).** Nine files report **zero** diagnostics — `lib/git.js`, `lib/index.js`, `lib/naming.js`, `lib/paths.js`, `lib/pointer.js`, `lib/receipts.js`, `lib/registry.js`, `lib/routing.js`, `scripts/verify-pack.mjs` — and three report exactly one each — `codex/server.mjs` (TS18047 `memory` possibly null), `lib/search.js` (TS2741), `scripts/run-tests.mjs` (TS2345). That is **twelve files for three fixes**, strictly better than the four files this task originally named, at nearly the same cost. `lib/index.js` is the plugin entry point and comes free. The remaining candidates are listed with their measured counts in spec §7.3; `lib/pending.js` (78) is the reason the ratchet stops here for now.
...
- [ ] **Step 2: Add `tsconfig.json`.** Use `allowJs:true`, `checkJs:false`, `noEmit:true`, `strict:false`, `module:'NodeNext'`, `moduleResolution:'NodeNext'`, `target:'ESNext'`, `types:['node']`, **`skipLibCheck:true`**, and `files` listing those twelve paths. Add `"types": "tsc --noEmit"`. Put `// @ts-check` on the first parseable line after any shebang — measured: a marker *before* the shebang is a hard `TS18026` + `TS1005` error, and `codex/prepare.mjs` is the file that has one. The test must fail if a marker disappears **and** if the two sets disagree in either direction: a marked file that is neither in `files` nor imported by a file in `files` is silently unchecked, which is the opt-in model's one real hole.

**Why `skipLibCheck:true` is required, not a shortcut (measured).** Without it, `lib/index.js`, `lib/tools.js` and `codex/server.mjs` report `TS2307` and `TS6200` **inside `node_modules/@deepseek-ai/dsh-llm/lib/types/content.d.ts`** — third-party types conflicting with each other, not source defects in this repository. It skips `.d.ts` checking only; suppressing a *source* diagnostic with it is still forbidden.
- [ ] **Step 3: Red/green each file in order.** Add the nine zero-error markers first (they must go green with no source change — if one does not, the measurement was wrong and that file belongs in a later tier), then the three single-error files: after adding a marker, run `npm run types -- --pretty false`; fix only its reported errors with accurate JSDoc or genuinely correct runtime guards. Do not use `@ts-ignore`, `@ts-nocheck`, `any` casts, or `skipLibCheck` to suppress a source error. If a candidate reveals a broad dependency error despite `checkJs:false`, stop and document it before reducing the opt-in set; the minimum deliverable is `paths.js` plus one other nontrivial module. Use `tsc --listFilesOnly` to confirm imported files are present in the program but do not yield diagnostics merely from being imported.
...
**Interfaces:** The architecture test owns a reviewed `Map<basename, layer>` and `Map<basename, maxLines>`; the hygiene test owns the README blob-hash check, the command-reference check, Task 4's type opt-in list, and the assertion that the marked set and `tsconfig.json`'s `files` set are equal in both directions.
