...
test('a transaction publishes creates and updates and appends exactly one receipt', async (t) => {
  ...
  assert.deepEqual(
    (await listPendingIndexNotifications(bindingA, { dataRoot })).map((n) => n.txId),
    [txId],
  )
...
test('a notifier that succeeds drains the queue inside runTransaction', async (t) => {
  ...
  const receipt = await runTransaction(bindingA, request(txId), {
    dataRoot,
    notifyIndex: async (value) => {
      seen.push(value.txId)
    },
  })
...
test('a failing notifier marks the receipt stale without undoing the vault write', async (t) => {
  ...
  const receipt = await runTransaction(bindingA, request(txId), {
    dataRoot,
    notifyIndex: async () => {
      throw Object.assign(new Error('index offline'), { code: 'EIO' })
    },
  })
...
test('a repeated idempotencyKey returns the original receipt and writes nothing', async (t) => {
  ...
  const first = await runTransaction(
    bindingA,
    request(newTransactionId(), { idempotencyKey: key }),
    { dataRoot },
  )
...
for (const [failAfter, outcome] of FAULT_POINTS) {
  test(`an interruption after ${failAfter} recovers to ${outcome}`, async (t) => {
    ...
    assert.deepEqual(
      report.unresolved,
      [],
      `recovery must resolve itself: ${JSON.stringify(report.unresolved)}`,
    )
    ...
    if (outcome === 'rollback') {
      ...
      assert.equal(
        await findReceipt(bindingA, key, { dataRoot }),
        null,
        'a rolled-back transaction must not answer its key',
      )
      assert.equal(
        await exists(at(vault, NEW_NOTE)),
        false,
        'a freshly created file must not survive a rollback',
      )
      ...
      assert.equal(
        await exists(at(vault, LOG_RELATIVE_PATH)),
        false,
        'no receipt may survive a rollback',
      )
      ...
      assert.equal(
        quarantined.length,
        1,
        `expected the created note under ${HISTORY_ROOT}/${txId}: ${entries}`,
      )
      assert.equal(
        await readFile(at(vault, `${HISTORY_ROOT}/${txId}/${quarantined[0]}`), 'utf8'),
        NEW_NOTE_TEXT,
      )
      ...
      assert.equal(
        await readFile(at(vault, `${HISTORY_ROOT}/${txId}/${snapshot}`), 'utf8'),
        STATUS_BEFORE,
      )
      ...
      const retry = await runTransaction(
        bindingA,
        request(retryTxId, { idempotencyKey: retryKey }),
        { dataRoot },
      )
      ...
      assert.equal(
        await exists(at(vault, `${HISTORY_ROOT}/${txId}`)),
        true,
...
test('an update whose bytes already match is skipped and keeps the mtime', async (t) => {
  ...
  assert.equal(await readFile(at(vault, STATUS_NOTE), 'utf8'), STATUS_BEFORE)
  assert.equal(
    (await stat(at(vault, STATUS_NOTE))).mtimeMs,
    before.mtimeMs,
    'a semantic no-op must not touch the mtime',
  )
  ...
  const receipt = await runTransaction(
    bindingA,
    {
      ...
      updates: [
        {
          path: MOC,
          transform: (current) => (current.equals(Buffer.from(MOC_BEFORE)) ? null : 'changed'),
        },
      ],
      receipt: null,
    },
...
test('a real size>0 && blocks===0 file is refused as offloaded', async (t) => {
  ...
  assert.equal(
    (await stat(at(vault, STATUS_NOTE))).size,
    4096,
    'the offloaded file must be left exactly as it was',
  )
  assert.equal(await exists(at(vault, LOG_RELATIVE_PATH)), false)
})
...
test('an external edit survives rollback and stops further automatic writes', async (t) => {
  const { vault, dataRoot, bindingA } = await fixture(t)
  const txId = newTransactionId()
  await assert.rejects(runTransaction(bindingA, request(txId), { dataRoot, failAfter: 'moc' }), {
  ...
  const copy = JSON.parse(
    await readFile(at(vault, `${HISTORY_ROOT}/${txId}/manifest.json`), 'utf8'),
  )
...
test('an interruption between publishing a file and journaling it still rolls back', async (t) => {
  ...
  assert.equal(
    await readFile(at(vault, NEW_NOTE), 'utf8'),
    NEW_NOTE_TEXT,
    'the fixture must really have published it',
  )
  ...
  assert.equal(
    await readFile(at(vault, MOC), 'utf8'),
    MOC_AFTER,
    'the fixture must really have replaced it',
  )
...
test('an unreadable target during rollback is a conflict, never "already undone"', async (t) => {
  ...
  // (a) injected through the documented read seam, on the one published target
  {
    ...
    const report = await recoverTransactions(bindingA, {
      dataRoot,
      io: {
        readFile: async () => {
          throw deadlock
    ...
    assert.equal(
      report.rolledBack.includes(txId),
      false,
      'an unreadable target must not be reported as rolled back',
    )
    assert.equal(report.unresolved.length, 1)
    assert.equal(report.unresolved[0].txId, txId)
    ...
    assert.equal(
      await exists(join(dataRoot, 'transactions', report.vaultHash, `${txId}.json`)),
      true,
...
test('a receipt-store failure after the commit point rolls forward, never back', async (t) => {
  ...
  assert.equal(
    await readFile(at(vault, STATUS_NOTE), 'utf8'),
    STATUS_AFTER,
    'a committed vault write must never be undone',
  )
  assert.equal(await readFile(at(vault, NEW_NOTE), 'utf8'), NEW_NOTE_TEXT)
  assert.equal(countOf(await readFile(at(vault, LOG_RELATIVE_PATH), 'utf8'), txId), 1)
  ...
  assert.equal(
    (await historyEntries(vault, txId)).some((name) => name.endsWith('log.md')),
    false,
  )
  assert.deepEqual(
    (await listPendingIndexNotifications(bindingA, { dataRoot })).map((entry) => entry.txId),
    [txId],
  )
  ...
  assert.equal(
    (await stat(at(vault, LOG_RELATIVE_PATH))).mtimeMs,
    before.mtimeMs,
    'a replay must not re-apply',
  )
})
...
test('a delivered notification still keeps the manifest until the receipt is stored', async (t) => {
  ...
  assert.equal(
    await markIndexNotified(bindingA, { dataRoot, txId }),
    false,
    'a manifest that still owes the receipt store is never dropped',
  )
  assert.equal(await exists(manifestPath), true)
...
test('a manifest whose txId traverses is refused, and nothing outside the vault is deleted', async (t) => {
  ...
  await assert.rejects(
    ...
    (error) =>
      error instanceof TransactionError &&
      error.code === 'manifest-corrupt' &&
      /unsafe txId/.test(error.message),
  )
  assert.equal(
    await exists(join(victim, 'keep.txt')),
    true,
    'a traversing txId must never delete outside the vault',
  )
  assert.equal(
    await exists(manifestFile),
    true,
    'the refused manifest is left for a human, not acted on',
  )
...
test('a manifest whose vaultHash traverses is refused, and nothing outside the data root is deleted', async (t) => {
  ...
  await assert.rejects(
    ...
    (error) =>
      error instanceof TransactionError &&
      error.code === 'manifest-corrupt' &&
      /unsafe vaultHash/.test(error.message),
  )
...
test('a txId whose history already exists is refused instead of overwritten', async (t) => {
  ...
  assert.ok(
    (await historyEntries(vault, txId)).some((name) => name.endsWith('ADR-1-调度器.md')),
    'the create was quarantined',
  )
...
test('two projects writing the registry and the log at once lose no update', async (t) => {
  ...
  const txA = {
    ...
    updates: [
      {
        path: REGISTRY,
        transform: addRow({ projectId: ID_A, dir: PROJECT_A, displayName: 'Alpha', remote: '' }),
      },
    ],
  ...
  const txB = {
    ...
    updates: [
      {
        path: REGISTRY,
        transform: addRow({ projectId: ID_B, dir: PROJECT_B, displayName: 'Beta', remote: '' }),
      },
    ],
    receipt: { action: 'bootstrap' },
  }
  ...
  assert.deepEqual(
    receipts.map((receipt) => receipt.result.status),
    ['applied', 'applied'],
  )
...
test('two projects binding at once both land their registry row', async (t) => {
  ...
  assert.deepEqual(
    results.map((result) => result.registryUpdated),
    [true, true],
  )
...
test('a committed transaction still queued for the index keeps its txId reserved', async (t) => {
  ...
  const txId = newTransactionId()
  await assert.rejects(
    runTransaction(bindingA, request(txId), { dataRoot, failAfter: 'index-notify' }),
    { code: 'injected-failure' },
  )
...
test('recovery works after a real process dies with the lock on disk', async (t) => {
  ...
  const failure = await execFileAsync(process.execPath, [script, vault, dataRoot, ID_A], {
    encoding: 'utf8',
  }).then(
    () => null,
    (error) => error,
  ...
  for (const entry of [
    {
      ...
      hash: sha256(agent),
      contents: 'rewritten',
      own: { id: 'dec-11111111-1111-4111-8111-111111111111' },
    },
    {
      path: STATUS_NOTE,
      transform: () => 'rewritten',
    ...
    runTransaction(
      bindingA,
      {
        ...
        updates: [
          {
            path: STATUS_NOTE,
            hash: sha256('没有 frontmatter 的文件\n'),
            contents: 'x',
            own: { id },
          },
        ],
...
test('unsafe, duplicate or already-taken targets are refused before the first write', async (t) => {
  ...
  const cases = [
    [
      'traversal',
      { txId: txId(), creates: [{ path: '../outside.md', contents: 'x' }] },
      'unsafe-path',
    ],
    [
      'absolute',
      { txId: txId(), creates: [{ path: '/tmp/evil.md', contents: 'x' }] },
      'unsafe-path',
    ],
    [
      'existing create',
      { txId: txId(), creates: [{ path: STATUS_NOTE, contents: 'x' }] },
      'target-exists',
    ],
    [
      'duplicate target',
    ...
    [
      'missing hash',
      { txId: txId(), updates: [{ path: STATUS_NOTE, contents: 'x' }] },
    ...
    [
      'missing parent',
      { txId: txId(), creates: [{ path: `${PROJECT_A}/新目录/笔记.md`, contents: 'x' }] },
  ...
  await assert.rejects(
    runTransaction(
      bindingA,
      { txId: txId(), creates: [{ path: `${PROJECT_A}/链接目录/x.md`, contents: 'x' }] },
      { dataRoot },
    ),
...
test('a transaction leaks no file descriptor and leaves no lock behind', async (t) => {
  ...
  assert.equal(
    new Set(counts).size,
    1,
    `a transaction must not leak a descriptor: ${counts.join(',')}`,
  )
...
test('a committed receipt that cannot be stored blocks a same-key retry instead of applying twice', async (t) => {
  ...
  assert.equal(
    await readFile(at(vault, REGISTRY), 'utf8'),
    registryAfterFirst,
    'the registry must not gain a second row',
  )
  assert.equal(
    await readFile(at(vault, LOG_RELATIVE_PATH), 'utf8'),
    logAfterFirst,
    'the log must not gain a second receipt',
  )
...
test('a filesystem without hard links still publishes a readable lock record', async (t) => {
  ...
  const again = await runTransaction(
    bindingA,
    {
      txId: newTransactionId(),
  ...
  assert.deepEqual(
    await readdir(join(dataRoot, 'locks')),
    [],
    'no staged lock record is left behind',
  )
})
...
test('no temporary file survives a refused transaction', async (t) => {
  ...
  await recoverTransactions(bindingA, { dataRoot })
  const stray = (await readdir(at(vault, `${PROJECT_A}/Decisions`))).filter((name) =>
    name.startsWith('.'),
  )
  assert.deepEqual(stray, [], 'staging files must be cleaned up by recovery')
  assert.deepEqual(
    (await readdir(join(dataRoot, 'transactions', sha256(await realpath(vault))))).filter((name) =>
      name.endsWith('.json'),
    ),
    [],
  )
