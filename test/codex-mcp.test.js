...
test("the MCP surface is the plugin's six tools, with schemas derived from TOOL_PARAMETERS", () => {
  ...
  for (const tool of tools) {
    ...
    assert.deepEqual(
      ...
      Object.keys(TOOL_PARAMETERS[tool.name]).sort(),
    )
  }
  // The required list is the author-facing `required: true` set, nothing more.
  for (const tool of tools) {
    const required = Object.entries(TOOL_PARAMETERS[tool.name])
...
test('jsonSchemaFor mirrors the author-facing DSL', () => {
  ...
  assert.deepEqual(jsonSchemaFor(undefined), {
    type: 'object',
    properties: {},
    required: [],
...
test(
  'a handshake, a write and a search round-trip against a throwaway vault',
  { timeout: 60000 },
  async (t) => {
...
test(
  "the client's roots decide the project, not the server's own cwd",
  { timeout: 60000 },
  async (t) => {
    const { home, dsh, vault, repo } = world()
    ...
    assert.deepEqual(
      readdirSync(join(vault, 'Projects')).map((name) => name.replace(/--.*$/, '')),
      ['demo-repo'],
    )
    assert.equal(
      existsSync(join(vault, 'Projects', readdirSync(join(vault, 'Projects'))[0], 'Decisions')),
      true,
    )
  },
)
...
test(
  'an unknown tool is an error result, and the server keeps answering',
  { timeout: 60000 },
  async (t) => {
    const { home, dsh, vault, repo } = world()
    const client = await connect({
...
test('the marketplace plugin is complete, and its generated .mcp.json matches the generator', () => {
  ...
  assert.equal(
    existsSync(join(plugin, 'skills', 'obsidian-mem', 'SKILL.md')),
    true,
    'the plugin must ship the skill',
  )
  const marketplace = JSON.parse(
    readFileSync(
      join(REPO, 'codex', 'marketplace', '.agents', 'plugins', 'marketplace.json'),
      'utf8',
    ),
  )
