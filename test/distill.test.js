...
async function queueIn(t) {
  ...
  return { queueRoot, persistOutput: (jobId, output) => persistJobOutput(jobId, output, { queueRoot }) }
}
...
function throwsCode(code) {
  return (error) => {
    assert.equal(error?.name, 'DistillError', `expected a DistillError, got ${error?.name}: ${error?.message}`)
...
test('a non-JSON model output is refused as not-json', () => {
  assert.throws(() => validateDistillation('I could not find anything durable.', JOB, CONFIG), throwsCode('not-json'))
...
test('an envelope with an extra field is refused, never silently ignored', () => {
  assert.throws(() => validateDistillation('{"items":[],"cost":0.5}', JOB, CONFIG), throwsCode('schema'))
  assert.throws(() => validateDistillation('{"items":[],"notes":"hello"}', JOB, CONFIG), throwsCode('schema'))
})
...
test('the brief example: a doc item is refused with a message naming the type', () => {
  ...
  for (const type of ['doc', 'glossary', 'session-log', 'hub', 'method', 'invariant']) {
    assert.throws(() => validateDistillation(json([decisionItem({ type })]), JOB, CONFIG), throwsCode('type'))
  }
})
...
test('an item missing any contract field is refused', () => {
  for (const field of ['type', 'title', 'body', 'tags', 'confidence', 'assertion', 'status', 'supersedesId', 'evidenceSeqs']) {
    const item = decisionItem()
    delete item[field]
    assert.throws(() => validateDistillation(json([item]), JOB, CONFIG), throwsCode('schema'), `missing ${field}`)
  }
})

test('malformed field values are refused', () => {
  assert.throws(() => validateDistillation(json([decisionItem({ title: '   ' })]), JOB, CONFIG), throwsCode('schema'))
  assert.throws(() => validateDistillation(json([decisionItem({ body: '' })]), JOB, CONFIG), throwsCode('schema'))
  assert.throws(() => validateDistillation(json([decisionItem({ tags: 'dsh-mem/decision' })]), JOB, CONFIG), throwsCode('schema'))
  assert.throws(() => validateDistillation(json([decisionItem({ tags: [''] })]), JOB, CONFIG), throwsCode('schema'))
  assert.throws(() => validateDistillation(json([decisionItem({ confidence: 'high' })]), JOB, CONFIG), throwsCode('schema'))
  assert.throws(() => validateDistillation(json([decisionItem({ confidence: 1.5 })]), JOB, CONFIG), throwsCode('schema'))
  assert.throws(() => validateDistillation(json([decisionItem({ confidence: -0.1 })]), JOB, CONFIG), throwsCode('schema'))
  assert.throws(() => validateDistillation(json([decisionItem({ assertion: 'verified' })]), JOB, CONFIG), throwsCode('schema'))
  assert.throws(() => validateDistillation(json([decisionItem({ status: 'done' })]), JOB, CONFIG), throwsCode('schema'))
  assert.throws(() => validateDistillation(json([decisionItem({ evidenceSeqs: '2' })]), JOB, CONFIG), throwsCode('evidence'))
  assert.throws(() => validateDistillation(json([decisionItem({ evidenceSeqs: [2.5] })]), JOB, CONFIG), throwsCode('evidence'))
})

test('an oversize title, body, tag list or item count is refused', () => {
  assert.throws(() => validateDistillation(json([decisionItem({ title: 'x'.repeat(201) })]), JOB, CONFIG), throwsCode('item-too-long'))
  assert.throws(() => validateDistillation(json([decisionItem({ body: 'x'.repeat(4001) })]), JOB, CONFIG), throwsCode('item-too-long'))
  assert.throws(
    () => validateDistillation(json([decisionItem({ tags: Array.from({ length: 17 }, (_, i) => `t${i}`) })]), JOB, CONFIG),
    throwsCode('item-too-long'),
  )
  const three = [decisionItem({ title: 'a' }), decisionItem({ title: 'b' }), decisionItem({ title: 'c' })]
  assert.throws(
...
test('evidence is mandatory: an empty evidenceSeqs list is refused', () => {
  assert.throws(() => validateDistillation(json([decisionItem({ evidenceSeqs: [] })]), JOB, CONFIG), throwsCode('evidence'))
})
...
test('a seq outside allowedEvents is refused even when it is inside the job range', () => {
  ...
  for (const seq of [3, 7, 0, 999]) {
    assert.throws(() => validateDistillation(json([decisionItem({ evidenceSeqs: [seq] })]), JOB, CONFIG), throwsCode('evidence'))
  }
  assert.throws(() => validateDistillation(json([decisionItem({ evidenceSeqs: [2, 7] })]), JOB, CONFIG), throwsCode('evidence'))
})

test('every allowed seq is accepted, and repeated seqs are refused', () => {
  assert.equal(onlyItem(json([decisionItem({ evidenceSeqs: [2, 4, 5, 6] })])).evidenceSeqs.length, 4)
  assert.throws(() => validateDistillation(json([decisionItem({ evidenceSeqs: [2, 2] })]), JOB, CONFIG), throwsCode('evidence'))
})
...
test('observed without an independently verified tool result is downgraded to inferred', () => {
  const narrative = onlyItem(json([decisionItem({ assertion: 'observed', status: 'proposed', evidenceSeqs: [6] })]))
  ...
  const failedTool = onlyItem(json([decisionItem({ assertion: 'observed', status: 'proposed', evidenceSeqs: [5] })]))
  assert.equal(failedTool.assertion, 'inferred')
})

test('observed with a verified tool result is retained', () => {
  const item = onlyItem(json([decisionItem({ assertion: 'observed', status: 'proposed', evidenceSeqs: [4] })]))
...
test('both trust downgrades can apply to the same candidate', () => {
  const item = onlyItem(json([decisionItem({ assertion: 'observed', status: 'accepted', evidenceSeqs: [6] })]))
  assert.equal(item.assertion, 'inferred')
  assert.equal(item.status, 'provisional')
  assert.deepEqual(item.downgrades, ['observed-without-verification', 'accepted-without-user-confirmation'])
})
...
test('low confidence goes to the inbox and cannot supersede an established conclusion', () => {
  const low = onlyItem(json([decisionItem({
    confidence: 0.5,
...
test('the resolved settings survive a resolved-settings hand-off (no silent default reset)', () => {
  ...
  const settings = distillSettings({ distill: { ...CONFIG.distill, maxItems: 1, minConfidence: 0.99 } })
  ...
  assert.deepEqual(distillSettings(settings).route, { provider: 'deepseek-official', model: 'deepseek-flash' })
  assert.throws(
    () => validateDistillation(json([decisionItem({ title: 'a' }), decisionItem({ title: 'b' })]), JOB, settings),
    throwsCode('too-many-items'),
  )
...
test('a validated item carries the contract plus the routing decision', () => {
  ...
  assert.deepEqual(Object.keys(item).sort(), [
    'assertion', 'body', 'confidence', 'downgrades', 'evidenceSeqs', 'inbox', 'status', 'supersedesId', 'tags', 'title', 'type',
  ])
...
test('the default settings are the design defaults when config is absent', () => {
  const item = onlyItem('{"items":[{"type":"gotcha","title":"x","body":"y","tags":[],"confidence":0.8,"assertion":"stated","status":"proposed","supersedesId":null,"evidenceSeqs":[2]}]}', JOB, undefined)
  assert.equal(item.inbox, false)
})
...
function textStream(text, usage = { inputTokens: 10, outputTokens: 20, totalTokens: 30, cacheReadTokens: 0, reasoningTokens: 0 }) {
...
test('one model call: explicit route, bounded input/output, no tool schema at all', async (t) => {
  ...
  assert.deepEqual(Object.keys(request).sort(), ['maxTokens', 'messages', 'model', 'provider', 'signal', 'system'])
  ...
  assert.deepEqual(result.usage, { inputTokens: 10, outputTokens: 20, totalTokens: 30, cacheReadTokens: 0, reasoningTokens: 0 })
})
...
test('identity is a prefix-shaped UUIDv4 and the idempotency key is session:toSeq:index', async (t) => {
  ...
  assert.deepEqual(result.items.map((item) => item.preassignedId.slice(0, 4)), ['dec-', 'got-', 'con-'])
  for (const item of result.items) assert.equal(isValidNoteId(item.preassignedId), true, item.preassignedId)
  assert.deepEqual(result.items.map((item) => item.idempotencyKey), [
    `${SESSION_ID}:${TO_SEQ}:0`,
    `${SESSION_ID}:${TO_SEQ}:1`,
    `${SESSION_ID}:${TO_SEQ}:2`,
  ])
...
test('the request input is clamped to distill.maxInputChars, keeping the newest text', async (t) => {  const { queueRoot, persistOutput } = await queueIn(t)
...
test('a service that cannot stream is treated as no service: deferred, not a crash', async (t) => {
  const { queueRoot, persistOutput } = await queueIn(t)
  await writeJobAtomic(queueRoot, jobFixture())
  const result = await runPendingJob(jobFixture(), { llm: { name: 'not-an-llm' }, config: CONFIG, persistOutput })
...
test('a job claiming validated without its items is refused, never re-distilled', async (t) => {
  ...
  await assert.rejects(
    () => runPendingJob(jobFixture({ state: 'validated', output: { state: 'validated', raw: '{}', items: null, usage: null } }), {
      llm: deadLlm,
      config: CONFIG,
      persistOutput,
    }),
...
test('a job claiming raw-durable without its text is refused, never re-distilled', async (t) => {
  const { queueRoot, persistOutput } = await queueIn(t)
  await writeJobAtomic(queueRoot, jobFixture({ state: 'raw-durable', output: { state: 'raw-durable', raw: null, items: null, usage: null } }))
  const deadLlm = recordingLlm(() => { throw new Error('the model must not be called again') })
  await assert.rejects(
    () => runPendingJob(jobFixture({ state: 'raw-durable', output: { state: 'raw-durable', raw: null, items: null, usage: null } }), {
      llm: deadLlm,
      config: CONFIG,
      persistOutput,
    }),
...
test('the raw output is durable before validation, and the validated items after', async (t) => {
  ...
  assert.deepEqual(barriers.map((entry) => entry.state), ['raw-durable', 'validated'])
  ...
  assert.deepEqual(barriers[1].usage, { inputTokens: 10, outputTokens: 20, totalTokens: 30, cacheReadTokens: 0, reasoningTokens: 0 })
...
test('the persisted metadata holds only measured tokens and no invented currency cost', async (t) => {
  ...
  assert.deepEqual(Object.keys(stored.output.usage).sort(), ['cacheReadTokens', 'inputTokens', 'outputTokens', 'reasoningTokens', 'totalTokens'])
...
test('an aborted stream carries no usage, and a stream without one persists null rather than zeros', async (t) => {
  ...
  const llm = recordingLlm(() => (async function* () {
    yield { type: 'block-start', index: 0, block: { type: 'text' } }
...
test('a validated job resumes from the persisted items without a second model call', async (t) => {
  const { queueRoot, persistOutput } = await queueIn(t)
  await writeJobAtomic(queueRoot, jobFixture())
  const payload = json([decisionItem(), decisionItem({ type: 'gotcha', title: '踩坑', status: 'proposed' })])
  ...
  assert.equal(deadLlm.calls.length, 0)
  assert.deepEqual(second.items.map((item) => item.preassignedId), first.items.map((item) => item.preassignedId))
  assert.deepEqual(second.items.map((item) => item.idempotencyKey), first.items.map((item) => item.idempotencyKey))
...
test('a raw-durable job re-validates the same bytes without a second model call', async (t) => {
  ...
  await writeJobAtomic(queueRoot, jobFixture({
    state: 'raw-durable',
    output: { state: 'raw-durable', raw: payload, items: null, usage: { inputTokens: 3, outputTokens: 4, totalTokens: 7 } },
  }))
...
test('a raw-durable job whose raw is invalid fails again without a model call, and stays raw-durable', async (t) => {
  ...
  const deadLlm = recordingLlm(() => { throw new Error('the model must not be called again') })
  const [reloaded] = await loadPending(queueRoot)
  await assert.rejects(() => runPendingJob(reloaded, { llm: deadLlm, config: CONFIG, persistOutput }), throwsCode('not-json'))
...
test('validation failure leaves the raw output durable in the job', async (t) => {
  ...
  await assert.rejects(() => runPendingJob(jobFixture(), { llm, config: CONFIG, persistOutput }), throwsCode('evidence'))
...
test('persistJobOutput refuses to overwrite a validated item set with different ids', async (t) => {
  ...
  await writeJobAtomic(queueRoot, jobFixture())
  const persisted = await persistJobOutput(JOB_ID, {
    state: 'validated',
    raw: '{}',
    items: [{ preassignedId: 'dec-5d46ff43-1bf8-496d-8b9f-c11e89d4e2aa', idempotencyKey: `${SESSION_ID}:7:0` }],
    usage: null,
  }, { queueRoot })
  assert.equal(persisted.state, 'validated')

  await assert.rejects(
    () => persistJobOutput(JOB_ID, {
      state: 'validated',
      raw: '{}',
      items: [{ preassignedId: 'dec-11111111-1111-4111-8111-111111111111', idempotencyKey: `${SESSION_ID}:7:0` }],
      usage: null,
    }, { queueRoot }),
  ...
  const again = await persistJobOutput(JOB_ID, {
    state: 'validated',
    raw: '{}',
...
test('persistJobOutput rejects a malformed barrier payload and a missing job', async (t) => {
  ...
  assert.equal(await persistJobOutput('job-ffffffffffffffffffffffffffffffff', { state: 'raw-durable', raw: '{}' }, { queueRoot }), null)
})
...
test('two racing validated barriers cannot both win: exactly one identity survives', async (t) => {
  ...
  const [left, right] = await Promise.allSettled([
    persistJobOutput(JOB_ID, { state: 'validated', raw: '{}', items: [item('dec-5d46ff43-1bf8-496d-8b9f-c11e89d4e2aa')], usage: null }, { queueRoot }),
    persistJobOutput(JOB_ID, { state: 'validated', raw: '{}', items: [item('dec-11111111-1111-4111-8111-111111111111')], usage: null }, { queueRoot }),
  ])
  ...
  const [stored] = await loadPending(queueRoot)
  assert.equal(stored.output.items.length, 1)
  assert.equal(stored.output.items[0].preassignedId, [left, right].find((entry) => entry.status === 'fulfilled').value.output.items[0].preassignedId)
})
...
test('a missing llm service defers and keeps the job byte-for-byte', async (t) => {
  ...
  const result = await runPendingJob(jobFixture(), { llm: undefined, config: CONFIG, persistOutput })
...
test('an empty route defers without ever calling the service (NO_ADAPTER is not a default route)', async (t) => {
  ...
  const llm = recordingLlm(() => { throw new Error('an empty route must not be dispatched') })
...
test('a tool call in the stream is a hard failure and nothing is persisted', async (t) => {
  ...
  await assert.rejects(() => runPendingJob(jobFixture(), { llm, config: CONFIG, persistOutput }), throwsCode('tool-call'))
  const [stored] = await loadPending(queueRoot)
...
test('a caller cancel is a terminal aborted chunk classified as aborted, never from message text', async (t) => {
  ...
  const llm = recordingLlm((options) => (async function* () {
    yield { type: 'finish', reason: { kind: 'aborted', failure: { code: 'ABORTED', message: sharedMessage } } }
    assert.equal(options.signal.aborted, true)
  })())

  await assert.rejects(
    () => runPendingJob(jobFixture(), { llm, config: CONFIG, signal: controller.signal, persistOutput }),
    throwsCode('aborted'),
  )
  const [stored] = await loadPending(queueRoot)
...
test('a timeout is the same terminal chunk with the same message, told apart only by signal.reason.name', async (t) => {
  ...
  await assert.rejects(
    () => runPendingJob(jobFixture(), { llm, config: { distill: { ...CONFIG.distill, timeoutMs: 20 } }, persistOutput }),
    throwsCode('timeout'),
  )
...
test('an empty-route terminal NO_ADAPTER chunk is reported as no-adapter', async (t) => {
  ...
  await writeJobAtomic(queueRoot, jobFixture())
  const llm = recordingLlm(() => (async function* () {
    yield { type: 'finish', reason: { kind: 'error', failure: { code: 'NO_ADAPTER', message: 'no adapter registered for provider ""' } } }
  })())

  await assert.rejects(() => runPendingJob(jobFixture(), { llm, config: CONFIG, persistOutput }), throwsCode('no-adapter'))
})
...
test('a max-tokens finish is a truncation refusal, and a missing finish is a refusal', async (t) => {
  ...
  const truncated = recordingLlm(() => (async function* () {
    yield { type: 'text-delta', index: 0, text: '{"items":[' }
    yield { type: 'finish', reason: { kind: 'max-tokens' } }
  })())
  await assert.rejects(() => runPendingJob(jobFixture(), { llm: truncated, config: CONFIG, persistOutput }), throwsCode('truncated'))

  const silent = recordingLlm(() => (async function* () {
    yield { type: 'block-start', index: 0, block: { type: 'text' } }
    yield { type: 'text-delta', index: 0, text: '{"items":[]}' }
  })())
  await assert.rejects(() => runPendingJob(jobFixture(), { llm: silent, config: CONFIG, persistOutput }), throwsCode('finish-missing'))

  const errored = recordingLlm(() => (async function* () {
    yield { type: 'finish', reason: { kind: 'error', failure: { code: 'MISSING_CREDENTIAL', message: 'no API key' } } }
  })())
  await assert.rejects(() => runPendingJob(jobFixture(), { llm: errored, config: CONFIG, persistOutput }), throwsCode('stream-error'))
  assert.equal((await loadPending(queueRoot))[0].output, null)
})
