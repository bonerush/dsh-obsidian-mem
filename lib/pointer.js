...
export function parsePointerBytes(bytes, { path = POINTER_FILENAME } = {}) {
  ...
  try {
    ...
    throw new PointerError('pointer-corrupt', `${path} is not valid JSON: ${error.message}`, {
      cause: error,
    })
  }
  ...
  if (!isUuidV4(value.projectId))
    throw new PointerError('pointer-corrupt', `${path} has no UUIDv4 projectId`)
  ...
  if (
    ...
    // eslint-disable-next-line no-control-regex -- a control character in a name is the corruption being checked for
...
export function newPointer(repoRoot) {
  ...
  const displayName =
    name
      // eslint-disable-next-line no-control-regex -- sanitising a directory name into a display name
      .replace(/[\u0000-\u001f\u007f]/g, ' ')
      .replace(/\s+/g, ' ')
      .trim()
