...
//     destination** (R33): `inbox: true` parks the note under `Inbox/` while the
...
/** Cross-project methodology notes are the one route outside `Projects/` (§6.2). */
export const METHOD_DIRECTORY = 'Methods'

/** `id` shape for every plugin-created note (§6.4): a known prefix plus a UUIDv4. */
const NOTE_ID_PATTERN =
  /^(?:doc|dec|got|con|log|hub|glo|met)-[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/
...
export function normalizeType(type) {
  ...
  if (!MEMORY_TYPES.includes(resolved)) {
    throw new RangeError(
      `type must be one of ${MEMORY_TYPES.join(', ')} (or the alias invariant): ${JSON.stringify(type)}`,
    )
  }
  return resolved
}
 ...
 * candidate is parked under `Inbox/` whatever its type, because the decision has
 ...
 * @param {object} binding - a `kind:'bound'` binding.
 * @param {string} type - requested type.
 * @param {{ inbox?: boolean }} [options] - the inbox destination, which owns `Inbox/index.md` (R33).
 ...
 *
 * `inbox: true` routes to `Inbox/` (R33) and consumes no ADR number, so parking a
...
export function routeNote(binding, type, title, options = {}) {
  ...
  if (kind === 'session-log')
    return `${directoryFor(binding, kind)}/${requireDay(today)}${NOTE_EXTENSION}`
  if (kind === 'hub') return `${directoryFor(binding, kind)}/index${NOTE_EXTENSION}`
  if (kind === 'glossary') return `${directoryFor(binding, kind)}/glossary${NOTE_EXTENSION}`
  if (kind === 'method')
    return `${METHOD_DIRECTORY}/${fitFilename(safeBasename(requireTitle(title), existingNames))}${NOTE_EXTENSION}`
...
export function appendGeneratedLine(text, line) {
  ...
  if (block === null) {
    throw new MemoryError(
      ...
      'the MOC has no `obsidian-mem:generated` region to write into',
    )
  }
  if (!block.matches) {
    throw new MemoryError(
...
function fitFilename(basename) {
  ...
  const kept = truncateToBytes(basename, budget)
    .replace(/^[\s.]+/u, '')
    .replace(/[\s.]+$/u, '')
...
function requireTitle(title) {
  if (typeof title !== 'string' || title.trim() === '')
    throw new RangeError('title must be a non-blank string')
  return title
}
