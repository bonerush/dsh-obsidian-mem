...
//   * Every emitted file name is bounded by the §6.4/R22 budget *including* its
//     `.md` extension (R28), so the routing layer can never hand the transaction
...
import { safeBasename } from './naming.js'
...
/** The §6.4/R22 whole-file budget, extension included (R28). */
 ...
 */
export function directoryFor(binding, type) {
 ...
 */
export function mocPathFor(binding, type) {
  const kind = normalizeType(type)
...
/**
 * Route one memory to its vault-relative note path (§6.2/R22/R28).
 *
 ...
 * @param {{ today?: string, adrNumber?: number, existingNames?: string[] }} [options] - clock date for a day log, ADR number, and the target directory's names.
...
export function routeNote(binding, type, title, options = {}) {
  const kind = normalizeType(type)
  const { today, adrNumber = 1, existingNames = [] } = options
...
/**
 * Fit a basename into the whole-file budget (R28).
 *
