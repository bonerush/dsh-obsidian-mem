...
//   * **`.history` growth is bounded, never blind.** Committed snapshots of
//     ordinary notes are the only recovery material for content that no longer
//     exists anywhere else, so retention keeps the newest `keepCount`
//     directories and everything younger than `maxAgeDays`, never removes
//     anything younger than `minAgeHours` (which is what makes racing a live
//     transaction impossible), and never removes a directory a
//     `needs-manual-repair` transaction still references. Pruning is an explicit
//     request, exactly like the report.
//
 ...
 * @param {boolean} [options.report] - explicitly create/update `_meta/Lint Report <date>.md`.
 * @param {boolean} [options.pruneHistory] - explicitly apply the `.history` retention policy.
...
export async function lintVault(options = {}) {
  ...
  const history = await checkHistory({
    binding, vaultRoot, dataRoot, home, policy, prune: pruneHistory, add,
  })
  throwIfAborted(signal)
 ...
 */
async function checkHistory({ binding, vaultRoot, dataRoot, home, policy, prune, add }) {
  const section = {
    policy: { ...policy },
    directories: 0,
    bytes: 0,
  ...
  const names = (await fs.readdir(absolute, { withFileTypes: true })).filter((entry) => entry.isDirectory() && !entry.isSymbolicLink())
  const now = Date.now()
  const inspected = []
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      section.pruned.push(candidate.txId)
    } catch (error) {
...
async function checkQueue(queueRoot, add) {
  const { jobs, invalid } = await readPendingJobs(queueRoot)
...
async function auditRepository(repoRoot, { matchers, linkNames, add }) {
  const walked = await walkTree(repoRoot, {
    matchers,
    maxFiles: 5_000,
...
export function renderLintReportBody(report) {
  ...
  lines.push('## .history 保留策略', '',
    ...
    `- 本次可清理 ${report.history.prunable} 个，已清理 ${report.history.pruned.length} 个`,
