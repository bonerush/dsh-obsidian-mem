...
export async function lintVault(options = {}) {
  ...
  if (
    (writeReport === true || pruneHistory === true) &&
  ...
  const filePaths = new Set(
    swept.entries.filter((entry) => entry.kind === 'file').map((entry) => entry.path),
  )
  ...
  const history = await checkHistory({
    ...
    policy,
    prune: pruneHistory,
    lockTimeoutMs: pruneLockTimeoutMs,
    add,
  })
  throwIfAborted(signal)
  const pending =
    queueRoot === null || queueRoot === undefined
...
function requireBoundBinding(binding) {
  ...
  if (binding.kind !== 'bound')
    throw new RangeError(`lintVault requires a bound binding, not ${JSON.stringify(binding.kind)}`)
...
function normalizeHistoryPolicy(overrides) {
  ...
  for (const [key, value] of Object.entries(overrides ?? {})) {
    if (!Number.isFinite(value) || value < 0)
      throw new RangeError(`historyPolicy.${key} must be a non-negative number`)
    if (!Object.hasOwn(HISTORY_POLICY, key))
      throw new RangeError(`unknown historyPolicy field: ${key}`)
    policy[key] = value
  }
  return policy
}
...
async function walkTree(
  root,
  { matchers, maxFiles = MAX_LINT_FILES, maxDepth = MAX_LINT_DEPTH, entry = null },
) {
  const entries = []
  let truncated = false
...
async function sweepVault(vaultRoot, { matchers }) {
  ...
  return {
    ...swept,
    notes: swept.entries.filter(
      (item) => item.kind === 'file' && isIndexableRelativePath(item.path),
    ),
  }
}
    ...
    try {
      ...
      if (stats.size > MAX_NOTE_BYTES) {
        ...
        record.error = new Error(
          `${note.path} is larger than the ${MAX_NOTE_BYTES}-byte note bound`,
        )
        records.push(record)
        continue
      }
...
function checkNotes({ records, add, today, filePaths, foldedFiles, linkNames }) {
  ...
  for (const record of records) {
    if (record.error !== null) {
      if (record.reason === 'oversize') {
        ...
        add(
          ...
          'warn',
          record.path,
          `cannot read ${record.path}: ${record.error.code ?? record.error.message}`,
        )
      } else {
        add(
          'broken-frontmatter',
          'error',
          record.path,
          `cannot parse ${record.path}: ${record.error.code ?? record.error.message}`,
        )
      }
      continue
    }
    ...
    if (note.hasFrontmatter === true && data.trust === 'agent') {
      const required = IDLESS_TYPES.has(data.type)
        ? REQUIRED_OWNED_FIELDS
        : ['id', ...REQUIRED_OWNED_FIELDS]
      ...
      if (missing.length > 0) {
        add(
          ...
          'warn',
          record.path,
          `${record.path} declares trust:agent but is missing ${missing.join(', ')}`,
        )
      }
    }
    const reviewAfter =
      note.hasFrontmatter === true && typeof data.review_after === 'string'
        ? data.review_after
        : null
    if (reviewAfter !== null && reviewAfter < today && !HISTORY_STATUSES.includes(data.status)) {
      add(
        ...
        'info',
        record.path,
        `${record.path} was due for review on ${reviewAfter}`,
      )
    }
        ...
        if (!resolvesLink(target, record.path, filePaths, foldedFiles)) {
          ...
          if (!seen.has(key)) {
            ...
            add(
              ...
              'warn',
              record.path,
              `${record.path} links to [[${target}]], which does not exist in this vault`,
            )
          }
        }
      }
...
function checkCaseFoldDuplicates({ records, add }) {
  ...
  for (const names of groups.values()) {
    ...
    add(
      ...
      'warn',
      paths[0],
      `names differing only by case make [[${[...names.keys()][0]}]] ambiguous: ${paths.join(', ')}`,
      { paths },
    )
  }
}
...
async function checkIndex({ index, vaultRoot, dataRoot, home, swept, indexableRecords, add }) {
  ...
  if (!section.ready || status?.scanning === true) {
    ...
    add(
      ...
      'info',
      null,
      'the memory index has not completed a scan, so its rows were not compared with the vault',
    )
    return section
  }
  if (swept.truncated) {
    ...
    add(
      ...
      'info',
      null,
      'the vault walk hit its bound, so the index comparison was skipped rather than guessed',
    )
    return section
  }
  ...
  for (const path of rows.paths) {
    if (!fileSet.has(path))
      add(
        ...
        'warn',
        path,
        `the index still holds a row for ${path}, which no longer exists in the vault`,
      )
  }
  for (const path of files) {
    if (!rowSet.has(path))
      add('orphan-file', 'warn', path, `${path} exists in the vault but has no row in the index`)
  }
  return section
}
...
async function readIndexRowPaths({ vaultRoot, dataRoot, home, status }) {
  if (typeof dataRoot !== 'string' || dataRoot === '')
    return { paths: null, reason: 'no-data-root' }
  if (status?.backend !== 'sqlite')
    return { paths: null, reason: `backend-${status?.backend ?? 'unknown'}` }
  let dbPath
  try {
...
async function checkHistory({
  ...
  const absolute = await resolveVaultFile(vaultRoot, HISTORY_RELATIVE_ROOT, { home }).catch(
    () => null,
  )
  if (absolute === null || (await lstatOrNull(absolute)) === null) return section
  const identity =
    dataRoot === null || dataRoot === undefined
      ? null
      : await vaultIdentity(binding, { home }).catch(() => null)
  let names
  try {
    names = (await fs.readdir(absolute, { withFileTypes: true })).filter(
    ...
    add(
      ...
      `cannot read ${HISTORY_RELATIVE_ROOT}: ${error.code ?? error.message}`,
    )
    return section
  }
  const now = Date.now()
  ...
  for (const entry of names) {
    ...
    try {
      ...
      add(
        'history-unreadable',
        'warn',
        `${HISTORY_RELATIVE_ROOT}/${entry.name}`,
        `cannot read ${HISTORY_RELATIVE_ROOT}/${entry.name}: ${error.code ?? error.message}`,
      )
    }
  }
  ...
  for (const [index, facts] of inspected.entries()) {
    ...
    if (facts.manifest !== null && facts.manifest.state === 'needs-manual-repair')
      section.needsRepair.push(facts.txId)
    const protectedByReference = await isTransactionReferenced(facts.txId, {
      facts,
      dataRoot,
      identity,
      binding,
    ...
    if (prunable)
      candidates.push({
        txId: facts.txId,
        path: `${HISTORY_RELATIVE_ROOT}/${facts.txId}`,
        bytes: facts.bytes,
      })
  }
  ...
  if (section.bytes > policy.oversizeBytes) {
    ...
    add(
      ...
      'warn',
      HISTORY_RELATIVE_ROOT,
      `.history holds ${section.bytes} bytes across ${section.directories} transactions, above the ${policy.oversizeBytes}-byte bound`,
    )
  }
  for (const oversized of section.oversizedSnapshots) {
    add(
      ...
      'warn',
      oversized.path,
      `${oversized.path} is ${oversized.bytes} bytes, above the ${policy.oversizedSnapshotBytes}-byte per-file bound`,
    )
  }
  for (const txId of section.needsRepair) {
    add(
      ...
      'error',
      `${HISTORY_RELATIVE_ROOT}/${txId}`,
      `transaction ${txId} needs manual repair; automatic writes refuse until it is resolved and its snapshots are never pruned`,
    )
  }
  ...
  if (section.pruned.length > 0) {
    add(
      ...
      'info',
      HISTORY_RELATIVE_ROOT,
      `pruned ${section.pruned.length} snapshot transaction(s) older than the retention policy: ${section.pruned.join(', ')}`,
    )
  }
  return section
}
 ...
 */
async function sweepHistoryUnderLock({
  binding,
  vaultRoot,
  dataRoot,
  home,
  ...
  try {
    return await withVaultLock(
      binding,
      async ({ identity }) => {
        const pruned = []
        for (const candidate of candidates) {
          const target = await resolveVaultFile(vaultRoot, candidate.path, { home }).catch(
            () => null,
          )
          ...
          if (await isTransactionReferenced(candidate.txId, { facts, dataRoot, identity, binding }))
            continue
          ...
          try {
            ...
            add(
              ...
              'warn',
              candidate.path,
              `could not prune ${candidate.path}: ${error.code ?? error.message}`,
            )
          }
        }
        return pruned
...
async function inspectHistoryDirectory(directory, policy) {
  ...
  for (const entry of entries) {
    ...
    if (stats.size > policy.oversizedSnapshotBytes) {
      oversized = {
        path: `${HISTORY_RELATIVE_ROOT}/${basename(directory)}/${entry.name}`,
        bytes: stats.size,
      }
    }
  }
...
async function checkQueue(queueRoot, add) {
  ...
  try {
    ...
    add(
      ...
      'warn',
      'pending',
      `cannot read the pending queue: ${error.code ?? error.message}`,
    )
  ...
  for (const entry of invalid) {
    add(
      ...
      'warn',
      `pending/${entry.file}`,
      `${entry.file} is not a readable pending job: ${entry.reason}`,
    )
  }
  return section
}
...
async function auditRepository(repoRoot, { matchers, linkNames, add }) {
  ...
  try {
    ...
    add(
      ...
      'warn',
      null,
      `the repository Markdown audit could not walk ${repoRoot}: ${error.code ?? error.message}`,
    )
  ...
  for (const file of walked.entries.filter((entry) => entry.kind === 'file')) {
    ...
    add(
      'unlinked-repo-markdown',
      'info',
      file.path,
      `${file.path} is repository Markdown that no vault note references; it is reported only and never moved`,
    )
  }
  return section
}
...
export function renderLintReportBody(report) {
  ...
  lines.push(
    '## 索引',
    '',
    `- 后端 ${report.index.backend ?? '未知'}，就绪 ${report.index.ready ? '是' : '否'}`,
  ...
  lines.push(
    '## 待处理队列',
    '',
    `- 任务 ${report.pending.jobs} 个，失败 ${report.pending.failed} 个，不可读 ${report.pending.invalid} 个`,
    '',
  )
...
async function writeLintReportNote(binding, report, { now, dataRoot, home }) {
    ...
    try {
      await runTransaction(
        binding,
        {
          ...
          creates: [{ path: relativePath, contents }],
          updates: [],
          receipt: null,
        },
        { dataRoot, home },
      ...
      if (error instanceof TransactionError) {
        return {
          status: 'conflict',
          path: relativePath,
          message: `the vault refused the report write (${error.code}): ${error.message}`,
        }
      }
      throw error
    }
  ...
  try {
    ...
    return {
      status: 'conflict',
      path: relativePath,
      message: `cannot read the existing ${relativePath}: ${error.code ?? error.message}`,
    }
  }
  ...
  if (block === null) {
    return {
      status: 'conflict',
      path: relativePath,
      message: `${relativePath} has no obsidian-mem:generated region; refusing to overwrite it`,
    }
  }
  if (!block.matches) {
    return {
      status: 'conflict',
      path: relativePath,
      message: `${relativePath} was edited by hand (its declared sha256 no longer matches its body); refusing to overwrite the human revision`,
    }
  }
            ...
            transform: (bytes) => {
              ...
              if (found === null || !found.matches) {
                throw new MemoryError(
                  ...
                  `${relativePath} changed while the report was being written; refusing to overwrite it`,
                )
              }
              if (found.body === body) return null
              return Buffer.from(
                `${text.slice(0, found.beginStart)}${generatedBlock(body)}${text.slice(found.endStart)}`,
                'utf8',
              )
            },
          },
 ...
 */
export async function weeklyLintHint({
  vaultPath,
  home = homedir(),
  now = new Date(),
  maxAgeDays = LINT_HINT_AGE_DAYS,
} = {}) {
  ...
  const reports = names.filter(
    (name) => name.startsWith(`${LINT_REPORT_PREFIX} `) && name.endsWith('.md'),
  )
  let newest = 0
  let newestName = null
  ...
  return {
    due,
    text: due ? hintText(lastLintAt) : '',
    lastLintAt,
    reports: reports.length,
    newestReport: newestName,
  }
}
