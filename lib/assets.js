 ...
 */
export function resolveSkillTargetDir({ dshHome = process.env.DSH_HOME ?? join(homedir(), '.dsh') } = {}) {
...
export async function syncSkill({ sourceDir, targetDir } = {}) {
  ...
  if (changed) {
    await writeFileAtomic(join(target, MANIFEST_FILENAME), Buffer.from(`${JSON.stringify({
...
async function readSourceAssets(sourceDir) {
  ...
  if (rootStat === null) throw new AssetSyncError(`the packaged skill directory does not exist: ${root}`)
  if (!rootStat.isDirectory()) throw new AssetSyncError(`the packaged skill source is not a directory: ${root}`)
...
async function collectAssets(root, prefix, into) {
  ...
  for (const entry of entries) {
    ...
    if (stat.isSymbolicLink()) {
      throw new AssetSyncError(`the packaged skill contains a symlink, which is never shipped: ${relative}`)
    }
    if (stat.isDirectory()) {
    ...
    if (stat.size > MAX_ASSET_BYTES) {
      throw new AssetSyncError(`the packaged asset is larger than ${MAX_ASSET_BYTES} bytes: ${relative}`)
    }
...
function validateSkillDocument(bytes, label) {
  ...
  try {
    ...
    throw new AssetSyncError(`${label} frontmatter is not valid YAML: ${error instanceof Error ? error.message : String(error)}`)
  }
  if (data === null || typeof data !== 'object' || Array.isArray(data)) {
    throw new AssetSyncError(`${label} frontmatter must be a mapping`)
  }
  if (data.name !== SKILL_NAME) {
    throw new AssetSyncError(`${label} frontmatter name must be "${SKILL_NAME}", found ${JSON.stringify(data.name)}`)
  }
...
async function readManifest(target) {
  ...
  try {
    ...
    if (source === null || typeof source !== 'object' || Array.isArray(source)) throw new Error('assets is not a mapping')
    const assets = {}
    for (const [key, value] of Object.entries(source)) {
      if (typeof value !== 'string' || !/^[0-9a-f]{64}$/.test(value)) throw new Error(`bad hash for ${key}`)
      assets[assetSegments(key).join('/')] = value
    }
    if (parsed.skill !== undefined && parsed.skill !== SKILL_NAME) throw new Error('written by another skill')
...
function sortAssets(assets) {
  return Object.fromEntries(Object.entries(assets).sort(([left], [right]) => (left < right ? -1 : left > right ? 1 : 0)))
}
