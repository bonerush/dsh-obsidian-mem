 ...
 * @param {{ truncated?: boolean }} [options] - `truncated: true` states that `bytes` is a
 *   prefix of a longer file, so its last line may be a fragment. A bounded reader must set
 *   it: a head cut right after a `---` would otherwise be read as a closing marker that
 *   the real file never contains.
 ...
 */
export function parseNote(bytes, { truncated = false } = {}) {
  const parsed = parseFrontmatterBytes(bytes, { truncated })
...
export function patchOwnedFields(bytes, changes, expectedHash) {
  ...
  if (!parsed.hasFrontmatter) {
    throw new FrontmatterError(
      'no-frontmatter',
      'the note has no frontmatter block at byte 0, so no field can be replaced in place',
    )
  }
  ...
  if (actualHash !== expectedHash) {
    throw new FrontmatterError(
      'hash-mismatch',
      'the note changed after it was read; refusing to overwrite a newer revision',
      {
  ...
  entries.forEach(([key, value], order) => {
    ...
    if (
      ...
      throw new FrontmatterError(
        'unlocatable-value',
        `${key} has no replaceable value range (an empty value would need the file rewritten)`,
      )
    }
...
export function assertOwnedFrontmatter(text, { path = 'note' } = {}) {
  ...
  try {
    ...
    throw new FrontmatterError(
      'template-invalid',
      `${path} was not rendered as a valid note: ${error.message}`,
      { cause: error },
    )
  }
  ...
  for (const [key, value] of Object.entries(note.data ?? {})) {
    if (!Object.hasOwn(OWNED_FIELD_TYPES, key)) {
      throw new FrontmatterError(
        'unknown-property',
        `${path} writes ${JSON.stringify(key)}, which is outside the §6.4 vocabulary`,
      )
    }
    try {
 ...
 */
export async function validateKnownPropertyTypes(
  vaultRoot,
  { home = homedir(), maxFiles = PROPERTY_PREFLIGHT_MAX_FILES } = {},
) {
  ...
  for (const relativePath of inventory.paths) {
    ...
    let parsed
    try {
      parsed = parseFrontmatterBytes(head.bytes, { truncated: head.truncated })
    } catch (error) {
 ...
 * @param {{ truncated?: boolean }} [options] - whether `input` is a prefix of a longer file.
 ...
 */
function parseFrontmatterBytes(input, { truncated = false } = {}) {
  if (typeof truncated !== 'boolean') throw new RangeError('truncated must be a boolean')
  ...
  if (bytes.length >= BOM.length && BOM.every((byte, index) => bytes[index] === byte)) {
    throw new FrontmatterError(
      'bom',
      'the note starts with a BOM; frontmatter must begin at byte 0 (spec §6.4/R16)',
    )
  }
  ...
  if (!Buffer.from(text, 'utf8').equals(bytes)) {
    throw new FrontmatterError(
      'invalid-utf8',
      'the note is not valid UTF-8, so character offsets cannot be mapped to bytes',
    )
  }
  ...
  // Two different things can cut the text short, and both make the last line in
  // the scanned window a fragment that must never be read as a closing marker:
  // the 64 KiB scan window itself, and a caller that handed us a head of a longer
  // file (`readNoteHead`). Deriving the second from `bytes.length` would miss it —
  // the head IS the whole buffer from this function's point of view.
  const windowTruncated = bytes.length > FRONTMATTER_SCAN_LIMIT
  const openEnded = windowTruncated || truncated
  const scanEndChar = charIndexForByteLimit(text, FRONTMATTER_SCAN_LIMIT)
  const closing = findClosingLine(text, contentStartChar, scanEndChar, openEnded)
  if (closing === null) {
    throw new FrontmatterError(
  ...
  if (duplicate !== undefined) {
    throw new FrontmatterError(
      'duplicate-key',
      `the frontmatter repeats a key: ${firstLine(duplicate.message)}`,
    )
  }
  if (document.errors.length > 0) {
    throw new FrontmatterError(
      'invalid-yaml',
      `the frontmatter is not valid YAML: ${firstLine(document.errors[0].message)}`,
    )
  }
  if (document.contents !== null && !isMap(document.contents)) {
    throw new FrontmatterError(
      'frontmatter-not-mapping',
      'the frontmatter is not a mapping of property names to values',
    )
  }
 ...
 * @param {boolean} openEnded - whether the text is cut short (by the window or by the caller's head).
 * @returns {{start: number, end: number}|null} character offsets of the marker line and of the body.
 */
function findClosingLine(text, from, scanEnd, openEnded) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    // The final element of a cut-short window is a fragment: it cannot prove a
    // line ended, so it can never be the closing marker.
    if (openEnded && index === lines.length - 1) continue
...
function trimToUtf8Boundary(bytes) {
  ...
  while (
    back < 3 &&
    bytes[bytes.length - back - 1] !== undefined &&
    (bytes[bytes.length - back - 1] & 0xc0) === 0x80
  ) {
    back += 1
  }
  if (back === 0) {
...
function renderToken(key, kind, value) {
  switch (kind) {
    ...
    case 'number-or-null': {
      ...
      if (key === 'confidence' && (value < 0 || value > 1))
...
function renderDate(key, value) {
  ...
  if (dateTime === null && date === null) {
    throw new RangeError(
      `${key} must be YYYY-MM-DD (or YYYY-MM-DD HH:mm:ss), never an ISO timestamp`,
    )
  }
...
function assertNoCommentInside(key, node) {
  ...
  for (const comment of collectCommentOffsets(node.srcToken)) {
    if (comment >= start && comment < end) {
      throw new FrontmatterError(
        'unpreservable-comment',
        `the value of ${key} contains a comment at character ${comment}, which cannot survive a single-token replacement`,
      )
    }
  }
}
...
function assertVisibleType(key, node, value) {
  ...
  const compatible = (() => {
    if (value === null)
      return kind === 'text-or-null' || kind === 'number-or-null' || kind === 'date-or-null'
    switch (kind) {
      case 'text':
      case 'text-or-null':
...
function assertPatchIsFaithful(parsed, output, changes) {
  ...
  try {
    ...
    throw new FrontmatterError(
      'self-check-failed',
      `the patched note no longer parses: ${error.message}`,
      { cause: error },
    )
  }
  ...
  for (const [key, value] of Object.entries(changes)) {
    if (!sameValue(patched[key], value)) {
      throw new FrontmatterError(
        'self-check-failed',
        `${key} did not round-trip to the requested value after patching`,
      )
    }
  }
  if (
    !output.subarray(verified.bodyStartByte).equals(parsed.bytes.subarray(parsed.bodyStartByte))
  ) {
    throw new FrontmatterError(
      'self-check-failed',
      'the patch changed the note body instead of only the target field',
    )
  }
}
