...
async function createNote(binding, request, options, { preassignedId }) {
  ...
  for (let attempt = 0; attempt < CREATE_ATTEMPTS; attempt += 1) {
    ...
    const plan = buildCreatePlan({ binding, request, kind, id, title, path, today, supersedes, contested, inbox })
    try {
 ...
 */
function buildCreatePlan({ binding, request, kind, id, title, path, today, supersedes, contested, inbox = false }) {
  ...
  if (supersedes !== null) {
    updates.push({
      ...
      // The hash the scan read: the engine refuses the update if the note moved
      // on between that read and the lock (spec: "当前哈希匹配").
      hash: supersedes.hash,
