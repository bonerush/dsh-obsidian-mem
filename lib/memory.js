...
//     inside the lock, so a note edited between the scan and the write is not
//     overwritten.
 ...
 *
 * @param {{ dataRoot?: string, home?: string, now?: Date }} [deps] - plugin data root, home for `~/` decisions, and the clock.
 * @returns {{dataRoot: string, home: string, now: Date}} the normalised deps.
 ...
 *
 * @param {object} binding - a `kind:'bound'` binding.
 * @param {object} request - the write request (`WriteRequest`).
 * @param {{ dataRoot?: string, home?: string, now?: Date }} [deps] - dependency seam.
...
async function createNote(binding, request, options, { preassignedId }) {
  ...
  for (let attempt = 0; attempt < CREATE_ATTEMPTS; attempt += 1) {
    ...
    // trying to take the one another writer just published.
    const adrNumber = kind === 'decision' ? await nextAdrNumber(binding, options) : 1
    const existingNames = kind === 'session-log' ? [] : await listNames(binding, directoryFor(binding, kind), options)
    const path = routeNote(binding, kind, title, { today, adrNumber, existingNames })
    const plan = buildCreatePlan({ binding, request, kind, id, title, path, today, supersedes, contested })
    try {
 ...
 */
function buildCreatePlan({ binding, request, kind, id, title, path, today, supersedes, contested }) {
  ...
  const moc = mocPathFor(binding, kind)
...
async function updateNote(binding, request, options) {
  ...
  const receipt = await runTransaction(
    binding,
    {
      txId: newTransactionId(),
      idempotencyKey: request.idempotencyKey ?? null,
      updates: [{ path: found.path, own: { id: request.id }, transform }],
      receipt: { action: 'write', sessionId: request.session ?? null },
    },
    { dataRoot, home },
  )
...
async function relatedNote(binding, id, options, field) {
  ...
  return { id, path: found.path, title: String(found.note.data.title ?? id) }
}
