...
//     `收件箱/` while its frontmatter keeps the type the item actually is — the
...
export function normalizeDeps(deps = {}) {
  if (deps === null || typeof deps !== 'object' || Array.isArray(deps)) throw new RangeError('deps must be an object')
  const dataRoot = deps.dataRoot ?? resolveDataRoot()
  if (typeof dataRoot !== 'string' || dataRoot.trim() === '') throw new RangeError('deps.dataRoot must be a non-blank path')
  const home = deps.home ?? homedir()
  if (typeof home !== 'string' || home.trim() === '') throw new RangeError('deps.home must be a non-blank path')
  const now = deps.now ?? new Date()
  if (!(now instanceof Date) || Number.isNaN(now.getTime())) throw new RangeError('deps.now must be a valid Date')
  const afterScan = deps.afterScan ?? null
  if (afterScan !== null && typeof afterScan !== 'function') throw new RangeError('deps.afterScan must be a function')
...
export function spliceBody(original, nextText, { today, changes = {} }) {
  ...
  return Buffer.concat([patched.subarray(0, patchedNote.bodyOffset), nextBytes.subarray(note.bodyOffset)])
}
...
export function assertPluginOwnedNote(note, vaultRelativePath, { expectId } = {}) {
  if (note === null || note.hasFrontmatter !== true) {
    throw new MemoryError('ownership-unproven', `${vaultRelativePath} has no frontmatter, so plugin ownership cannot be proven`)
  }
  const data = note.data ?? {}
  if (data.trust === 'owner') {
    throw new MemoryError('human-owned', `${vaultRelativePath} declares trust:owner; the plugin never rewrites a human-owned note`)
  }
  if (expectId !== undefined && data.id !== expectId) {
    throw new MemoryError('ownership-mismatch', `${vaultRelativePath} carries id ${JSON.stringify(data.id)}, not ${JSON.stringify(expectId)}`)
  }
...
export async function findNoteById(binding, id, deps = {}) {
  if (typeof id !== 'string' || id.trim() === '') throw new RangeError('id must be a non-blank string')
...
export async function readNoteById(binding, id, deps = {}) {
  ...
  if (found === null) throw new MemoryError('note-not-found', `no note in ${requireProjectDirectory(binding)} carries id ${JSON.stringify(id)}`)
  return { id, path: found.path, frontmatter: found.note.data, body: found.note.body, hash: found.hash }
}
 ...
 *
 * A create accepts `inbox: true` to park a low-confidence candidate in `收件箱/`
...
export async function createMemoryWithId(binding, request, deps = {}) {
  ...
  if (!isValidNoteId(preassignedId)) {
    throw new RangeError(`preassignedId must be "<known prefix>-<UUIDv4>": ${JSON.stringify(preassignedId)}`)
  }
  const kind = normalizeType(request.type)
  if (typePrefix(kind) !== preassignedId.slice(0, 3)) {
    throw new RangeError(`preassignedId ${preassignedId} does not carry the ${typePrefix(kind)}- prefix of type ${kind}`)
  }
  const replay = await replayReceipt(binding, request.idempotencyKey, options)
  if (replay !== null) return replay
 ...
 * touched: the same fact is created in the explicit inbox destination (`收件箱/`,
  ...
  if (supersedesId !== null) {
    ...
    if (found === null) {
      ...
      try {
        ...
        conflicts.push({ supersedesId, reason: typeof error?.code === 'string' ? error.code : 'ownership-unproven' })
        supersedesId = null
      }
...
/** The cross-project methodology directory (§6.2): the one route outside `项目/`. */
const METHOD_DIRECTORY_PREFIX = '方法/'

/**
 * Promote one project note into a new cross-project `方法/` note.
 *
 ...
 * @throws {MemoryError} with code `already-method`, `source-frontmatter-broken` or `note-not-found`.
...
/**
 * Append one session block to a day's cold log (`日志/YYYY-MM-DD.md`, §6.2).
 *
...
export async function appendLog(binding, request, deps = {}) {
  ...
  const section = request.section === undefined || request.section === null ? '会话' : requireText(request.section, 'section')
  const text = requireText(request.text, 'text')
  const discriminator = request.seq === undefined || request.seq === null ? section : String(request.seq)
  ...
  return runTransaction(
    binding,
    {
      ...
      receipt: { action: 'log', sessionId: session, fromSeq: numericSeq(request.seq), toSeq: numericSeq(request.seq) },
    },
    { dataRoot, home },
  )
}
...
async function createNote(binding, request, options, { preassignedId }) {
  ...
  const supersedesHash = supersedes !== null && typeof request.expectedSupersedesHash === 'string' && request.expectedSupersedesHash !== ''
  ...
  for (let attempt = 0; attempt < CREATE_ATTEMPTS; attempt += 1) {
    ...
    const existingNames = kind === 'session-log' ? [] : await listNames(binding, destination, options)
    const path = routeNote(binding, kind, title, { today, adrNumber, existingNames, inbox })
    const plan = buildCreatePlan({ binding, request, kind, id, title, path, today, supersedes, contested, inbox, supersedesHash })
    try {
 ...
 */
function buildCreatePlan({ binding, request, kind, id, title, path, today, supersedes, contested, inbox = false, supersedesHash = null }) {
  const fields = newNoteFields({ binding, request, kind, id, title, today, supersedes, contested })
  const links = []
  if (supersedes !== null) links.push(`- 取代 [[${noExtension(supersedes.path)}|${supersedes.title}]]。\n`)
  if (contested !== null) links.push(`- 与 [[${noExtension(contested.path)}|${contested.title}]] 并存（contested，双方证据保留）。\n`)
  ...
  if (moc !== null) updates.push({ path: moc, transform: generatedLineAppender(moc, wikilinkLine(path, title), today) })
...
function newNoteFields({ binding, request, kind, id, title, today, supersedes, contested }) {
  ...
  const tags = request.tags === undefined || request.tags === null
    ? (project === null ? [`dsh-mem/${kind}`] : [`dsh-mem/${kind}`, `project/${binding.slug}`])
    : request.tags
  return {
    id,
    type: kind,
...
function relinkTransform(target, { today, changes, linkLine }) {
  return (current) => {
    if (current === null) {
      throw new MemoryError('note-missing', `${target.path} disappeared before the transaction could update it`)
    }
    const note = parseNote(current)
    ...
    const patchedNote = parseNote(patched)
    const next = Buffer.concat([patched.subarray(0, patchedNote.bodyOffset), Buffer.from(`${note.body}${linkLine}`, 'utf8')])
    return next.equals(current) ? null : next
  }
}
...
function generatedLineAppender(mocPath, line, today) {
  return (current) => {
    if (current === null) {
      throw new MemoryError('moc-missing', `${mocPath} is missing; bootstrap the project before writing memories`)
    }
    const note = parseNote(current)
...
async function updateNote(binding, request, options) {
  ...
  if (request.type !== undefined && request.type !== null && normalizeType(request.type) !== kind) {
    throw new MemoryError('type-mismatch', `${found.path} is a ${kind} note; a type change is a new fact, not an update`)
  }
  const transform = updateTransform({ path: found.path, id: request.id, request, today })
  if (options.afterScan !== null) await options.afterScan({ path: found.path, id: request.id, hash: found.hash })
...
function updateTransform({ path, id, request, today }) {
  return (current) => {
    if (current === null) {
      throw new MemoryError('note-missing', `${path} disappeared before the transaction could update it`)
    }
    const note = parseNote(current)
    ...
    const patched = Object.keys(changes).length === 0 ? current : patchOwnedFields(current, changes, sha256Hex(current))
    if (!bodyChanged) return patched.equals(current) ? null : patched
    const patchedNote = parseNote(patched)
    const next = Buffer.concat([patched.subarray(0, patchedNote.bodyOffset), Buffer.from(nextBody, 'utf8')])
    return next.equals(current) ? null : next
  }
}
...
async function replayReceipt(binding, idempotencyKey, options) {
  ...
  const path = (stored.paths ?? []).find((candidate) => candidate.endsWith('.md') && candidate !== LOG_RELATIVE_PATH)
  if (path === undefined) {
    throw new MemoryError('receipt-corrupt', `receipt ${stored.txId} names no note path, so the replay cannot be resolved`)
  }
  const found = await findNoteByPath(binding, path, options)
  if (found === null || typeof found.note?.data?.id !== 'string') {
    throw new MemoryError('receipt-note-missing', `receipt ${stored.txId} names ${path}, which is no longer a plugin note in the vault`)
  }
...
async function relatedNote(binding, id, options, field) {
  ...
  if (typeof id !== 'string' || id.trim() === '') throw new RangeError(`${field} must be a non-blank id`)
  const found = await findNoteById(binding, id, options)
  if (found === null) throw new MemoryError('note-not-found', `${field} names ${id}, which does not exist in this project`)
...
function insertLogBlock(noteText, heading, text, marker) {
  ...
  if (next === -1) return noteText.endsWith('\n') ? `${noteText}${fragment}` : `${noteText}\n${fragment}`
...
function numericSeq(seq) {
  return Number.isSafeInteger(seq) ? seq : (typeof seq === 'string' && /^\d+$/.test(seq) ? Number(seq) : null)
}
