...
export function createMemoryServices(options = {}) {
  ...
  return {
    ...
    async admin(args, signal, exec) {
      ...
      return { action: 'jobs', result: await jobsAction(args, { queueRoot, diagnostics }) }
    },
 ...
 *
 * @param {object} args - the forwarded `mem_admin` arguments.
 * @param {{queueRoot: string, diagnostics?: object}} seams - the queue root and the decisions ring.
 * @returns {Promise<object>} the `jobs` result.
 */
async function jobsAction(args, { queueRoot, diagnostics }) {
