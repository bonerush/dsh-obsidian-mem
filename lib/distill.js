...
export function distillCandidates(raw, job, config) {
  ...
  if (typeof job.projectId !== 'string' || job.projectId === '') {
    throw new DistillError('job-invalid', 'a distillation job must name its project so a foreign supersede target can be refused')
  }
  ...
  for (let index = 0; index < items.length; index += 1) {
    const verdict = validateItem(items[index], index, { allowed, projectId: job.projectId, settings })
...
function validateItem(item, index, { allowed, projectId, settings }) {
  ...
  if (target.foreign === true) {
    return { refused: { index, reason: 'foreign-target', field: 'supersedesId', value: item.supersedesId } }
  }
  ...
  return {
    item: { type: item.type, title, body, tags, confidence, assertion, status, supersedesId, evidenceSeqs, inbox, downgrades },
  }
}
...
function supersedeTarget(value, index, projectId) {
  ...
  throw new DistillError('schema', `distill item ${index}: supersedesId must be null or a "<prefix>-<UUIDv4>" id (got ${describe(value)})`)
}
 ...
 * namespace, a bare `决策/ADR-2.md` project-relative reference and an `https://`
 ...
 * (`项目/<slug>--<id8>/`), which is the §5.1 naming rule.
 */
function isForeignPath(value, projectId) {
  ...
  for (const token of value.split(/[\s"'`()[\]{}<>,;，。；、]+/u)) {
    ...
    if (/^(?:_meta|方法)(?:[\\/]|$)/.test(token)) return true
...
export async function runPendingJob(job, { llm, config, signal, persistOutput } = {}) {
  ...
  if (persisted?.state === 'validated') {
    ...
    return { items: persisted.items, usage: persisted.usage ?? null, durationMs: elapsed(), refused: [] }
  }
