...
//     only, `global` is the cross-project `方法/` directory plus the read-only
...
export function resolveScope(input = {}) {
  ...
  if (scope === 'global') {
    // `global` has no project dimension at all: it is `方法/` plus the user's
 ...
 * @returns {Promise<object[]>} hits, best first, each carrying `path`, `title`, `snippet`, `signals`.
 * @throws {IndexError} when the index is not ready, unbound for `project`, or bound elsewhere.
