...
export async function vaultIdentity(binding, { home = homedir() } = {}) {
  ...
  if (binding.kind !== undefined && binding.kind !== 'bound') {
    throw new ReceiptError(
      'binding-invalid',
      `expected a bound binding, not ${JSON.stringify(binding.kind)}`,
    )
  }
...
export function renderLogEntry(receipt) {
  ...
  if (receipt.sessionId !== null && receipt.sessionId !== undefined) {
    lines.push(
      `- session: \`${receipt.sessionId}\` seq ${receipt.fromSeq ?? '?'}–${receipt.toSeq ?? '?'}`,
    )
  }
  const { status, created = 0, updated = 0, skipped = 0, index = 'none' } = receipt.result ?? {}
  lines.push(
    `- result: ${status}（新建 ${created}，更新 ${updated}，跳过 ${skipped}；索引 ${index}）`,
  )
  lines.push('- paths:')
...
export async function readReceiptRecord(path) {
  ...
  try {
    ...
    throw new ReceiptError(
      'receipt-unreadable',
      `cannot read the receipt at ${path}: ${error.code ?? error.message}`,
      { cause: error },
    )
  }
  try {
    ...
    throw new ReceiptError('receipt-corrupt', `the receipt at ${path} is not valid JSON`, {
      cause: error,
    })
  }
}
...
export async function findReceipt(
  binding,
  key,
  { dataRoot = resolveDataRoot(), home = homedir() } = {},
) {
