...
export async function openIndex(options = {}) {
  ...
  if (!Number.isInteger(scanBranchMaxRows) || scanBranchMaxRows < 1) throw new RangeError('scanBranchMaxRows must be a positive integer')
  if (!Number.isInteger(scanBranchPageSize) || scanBranchPageSize < 1) throw new RangeError('scanBranchPageSize must be a positive integer')
...
const NOTE_COLUMNS = 'n.path, n.hash, n.id, n.type, n.title, n.status, n.project, n.project_dir, n.project_id8, n.updated, n.source, n.broken'
...
async function search(state, options = {}) {
  ...
  if (signal !== null && signal.aborted === true) throw new IndexError('aborted', 'the search was cancelled before it started')
  const plan = planQuery(query)
  const windowSize = Math.min(MAX_CANDIDATES, Math.max(MIN_CANDIDATES, limit * CANDIDATE_FACTOR))

  const source = state.backend === 'sqlite'
  ...
  while (accepted.length < windowSize) {
    if (signal !== null && signal.aborted === true) throw new IndexError('aborted', 'the search was cancelled')
...
function sqliteTokenSource(state, { plan, filters, windowSize }) {
  ...
  const rows = match === null ? [] : state.db.prepare(`
...
function sqliteSubstringSource(state, { plan, filters, windowSize }) {
  ...
  let done = false
  return {
    get examined() { return examined },
    get truncated() { return truncated },
    get truncationReason() { return reason },
    page() {
      const rows = []
...
function scanTokenSource(state, { plan, filters, windowSize }) {
  ...
  candidates.sort((left, right) => right.matchedRatio - left.matchedRatio || comparePath(left.path, right.path))
...
function status(state) {
  return {
    ...
    lastFullVerify: state.lastFullVerify === 0 ? null : new Date(state.lastFullVerify).toISOString(),
