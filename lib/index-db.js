...
/**
 * Stable path comparison, so a source with no relevance signal still has a total order.
 *
 ...
 * list of hits while a caller can ask why the list is what it is.
 *
 * @param {object[]} hits - the sliced hits.
 * @param {{truncated: boolean, truncationReason: string|null, rowsExamined: number, candidatesScored: number}} meta - the search facts.
...
function openSqliteSource(state, { plan, filters, windowSize }) {
  return plan.branch === 'tokens'
    ...
    : sqliteSubstringSource(state, { plan, filters })
}
...
/**
 * The bounded substring branch: page through the *filtered* rows in path order.
 *
 * FTS5 cannot express a single CJK character, so the source reads rows itself.
 * Ordering by path (keyset pagination, not `OFFSET`, so a concurrent write cannot
 * shift the window) makes the bound a deterministic, repeatable prefix — the same
 * prefix the scan backend walks. The bound is only reported as truncation once a
 * further row proves it was the bound, not the end of the vault.
 *
 * @param {object} state - index state.
 * @param {{plan: object, filters: object}} input - query plan and filters.
 * @returns {CandidateSource} the source.
 */
function sqliteSubstringSource(state, { plan, filters }) {
  ...
  return {
    ...
    get truncationReason() { return truncated ? `substring-scan-bound:${bound}` : null },
    page() {
      if (done || truncated) return { rows: [], done: true }
      const room = Math.min(pageSize, bound - examined)
      if (room <= 0) {
        truncated = state.db.prepare(probeSql).get(...params, lastPath) !== undefined
        done = true
        return { rows: [], done: true }
      }
      const rows = state.db.prepare(pageSql).all(...params, lastPath, room)
      if (rows.length === 0) {
        done = true
        return { rows: [], done: true }
      }
      ...
      if (rows.length < room) done = true
      const matches = rows
        .filter((row) => `${row.title ?? ''}\n${row.body}`.toLowerCase().includes(needle))
        .map(toCandidate)
      return { rows: matches, done: done || truncated }
    },
  }
}
...
function openScanSource(state, { plan, filters, windowSize }) {
  return plan.branch === 'tokens'
    ...
    : scanSubstringSource(state, { plan, filters })
}
 ...
 * @returns {CandidateSource} the source.
 */
function scanSubstringSource(state, { plan, filters }) {
  const needle = plan.needle.toLowerCase()
  ...
  return {
    ...
    page() {
      ...
      const room = Math.min(pageSize, bound - examined)
      if (room <= 0) {
        truncated = cursor < ordered.length
        done = true
        return { rows: [], done: true }
      }
      ...
      const matches = slice
        .filter((record) => record.searchText.toLowerCase().includes(needle))
        .map((record) => ({ ...record, rank: null, matchedRatio: 0 }))
      return { rows: matches, done: done || truncated }
    },
  }
}
