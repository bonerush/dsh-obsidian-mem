...
/** Rows fetched per page by the substring branch; the bound still applies to the total. */
export const SCAN_BRANCH_PAGE_SIZE = 256
...
export const METHOD_PREFIX = 'Methods/'
...
export function planQuery(query) {
  ...
  const branch =
    view.bigrams.length === 0 && (view.loneCjk.length > 0 || view.tokens.length === 0)
      ? 'substring'
      : 'tokens'
...
export function buildMatchQuery(tokens) {
  const usable = tokens
    .filter(
      (token) => typeof token === 'string' && token.length > 0 && token.length <= MAX_TOKEN_CHARS,
    )
...
/**
 * The project directory (`Projects/<slug>--<projectId 前8位>`) a path lives under.
 *
...
export async function readNote(vaultRoot, relativePath, section, options = {}) {
  ...
  if (stats.size > BigInt(maxBytes)) {
    throw new IndexError(
      'note-too-large',
      `${relativePath} is ${stats.size} bytes, over the ${maxBytes}-byte note bound`,
    )
  }
...
export function extractSection(body, section) {
  ...
  if (start < 0)
    throw new IndexError('section-not-found', `no heading ${JSON.stringify(wanted)} in the note`)
...
export async function indexFilePath(vaultRoot, dataRoot, options = {}) {
  ...
  if (isInside(directoryReal, vault.root) || isInside(vault.root, directoryReal)) {
    throw new IndexError(
      'index-inside-vault',
      `the index directory ${directory} must not live inside the vault ${vault.root}`,
    )
  }
 ...
 * @param {number} [options.fullVerifyIntervalMs] - how stale `lastFullVerify` may get before a refresh re-hashes every source (evaluated per refresh; no timer).
 * @param {number} [options.scanBranchMaxRows] - rows the substring branch may examine (plan-mandated bound).
 * @param {number} [options.scanBranchPageSize] - rows fetched per page by that branch.
...
export async function openIndex(options = {}) {
  const {
    ...
    scanBranchMaxRows = SCAN_BRANCH_MAX_ROWS,
    scanBranchPageSize = SCAN_BRANCH_PAGE_SIZE,
    yieldToEventLoop,
    openDatabase,
    io,
  } = options
  ...
  if (!['auto', 'sqlite', 'scan'].includes(backend))
    throw new RangeError("backend must be one of 'auto', 'sqlite', 'scan'")
  ...
  if (!Number.isInteger(scanBranchMaxRows) || scanBranchMaxRows < 1)
    throw new RangeError('scanBranchMaxRows must be a positive integer')
  if (!Number.isInteger(scanBranchPageSize) || scanBranchPageSize < 1)
    throw new RangeError('scanBranchPageSize must be a positive integer')
  ...
  const state = {
    ...
    scanBranchPageSize,
    yieldToEventLoop:
      typeof yieldToEventLoop === 'function'
        ? yieldToEventLoop
    ...
    try {
      ...
      if (backend === 'sqlite') {
        throw error instanceof IndexError
          ...
          : new IndexError(
              'sqlite-unavailable',
              `the sqlite index backend is unavailable: ${error.message}`,
              { cause: error },
            )
      }
      state.backend = 'scan'
...
async function quarantine(state, reason) {
  ...
  for (const suffix of ['-wal', '-shm']) {
    ...
    if ((await lstatOrNull(sidecar)) !== null)
      await fs.rename(sidecar, `${moved}${suffix}`).catch(() => {})
  }
 ...
 *
 * @param {object} state - index state.
 * @param {{full?: boolean}} [options] - `full: true` re-hashes every source (the interval is evaluated per call; no timer exists).
 * @returns {Promise<object>} the refresh summary.
 */
async function runRefresh(state, options = {}) {
  ...
  // later pass re-hashes every source once `fullVerifyIntervalMs` has elapsed.
  // The interval is evaluated **per refresh call** — there is no timer here, so
  // the caller (or a transaction notification) must drive refreshes for an
  // mtime/size-preserving external edit to be caught without a matching hit
  // (spec §7 "定期校验"). `status().lastFullVerify` tells a caller when to ask.
  const full =
    requestedFull ||
  ...
  try {
    ...
    if (state.db !== null) {
      state.db
        .prepare("INSERT OR REPLACE INTO kv(key, value) VALUES ('last_scan', ?)")
        .run(state.lastScan)
    }
...
async function scanInto(state, { full }) {
  ...
  for (const relative of files) {
    ...
    try {
      ...
      throw new IndexError(
        'scan-failed',
        `cannot stat ${relative}: ${error.code ?? error.message}`,
        { cause: error },
      )
    }
    ...
    try {
      ...
      throw new IndexError(
        'scan-failed',
        `cannot read ${relative}: ${error.code ?? error.message}`,
        { cause: error },
      )
    }
...
async function listCandidateFiles(state) {
  ...
  while (queue.length > 0 && !truncated) {
    ...
    const absolute =
      directory === '' ? state.vaultRoot : join(state.vaultRoot, ...directory.split('/'))
    let entries
    try {
      ...
      throw new IndexError(
        'scan-failed',
        `cannot read vault directory ${directory || '.'}: ${error.code ?? error.message}`,
        { cause: error },
      )
    }
...
function storeRecord(state, record) {
  ...
  db.prepare('INSERT INTO notes_fts(rowid, title, body, tokens) VALUES (?, ?, ?, ?)').run(
    row.rowid,
    record.title ?? '',
    record.body,
    record.tokens.join(' '),
  ...
  const insertLink = db.prepare(
    'INSERT INTO links(src_id, target, resolved_id) VALUES (?, ?, NULL)',
  )
...
function touchStamp(state, relative, stamp) {
  ...
  state.db
    .prepare('UPDATE notes SET mtime = ?, size = ? WHERE path = ?')
    .run(stamp.mtime, stamp.size, relative)
}
...
function resolveLinks(state) {
  ...
  for (const note of db.prepare('SELECT path, id, title FROM notes').all()) {
    ...
    if (typeof note.title === 'string' && !byName.has(note.title))
      byName.set(note.title, note.id ?? note.path)
  }
...
/** The `notes` columns every candidate query selects, so both sources agree on the row shape. */
const NOTE_COLUMNS =
  'n.path, n.hash, n.id, n.type, n.title, n.status, n.project, n.project_dir, n.project_id8, n.updated, n.source, n.broken'

/**
 * Run one search against the active backend.
 *
 * The candidate window is scored **in full** and only then sliced to `limit`, so
 * the composite score (title / verbatim phrase / type / freshness) can promote a
 * note that bm25 alone would have left below the cut. The plan-mandated substring
 * bound can genuinely cut recall, so the returned array carries non-enumerable
 * `truncated`, `truncationReason`, `rowsExamined` and `candidatesScored`
 * properties: an empty array with `truncated: true` says "a bound stopped this
 * search", and only `truncated: false` says "searched and found nothing".
 *
 * `rowsExamined` is only comparable between backends on the substring branch,
 * where both walk the same path-ordered prefix and the plan-mandated bound
 * applies to exactly that number. On the tokens branch the source is a single
 * relevance-ordered window, so SQLite reports the rows it fetched (at most
 * `candidateWindow + 1`) while the in-memory scan reports every matching record.
 *
 * @param {object} state - index state.
 * @param {{query: string, filters?: object, limit?: number, signal?: AbortSignal}} options - the query, a resolved filter descriptor, a limit and caller cancellation.
 * @returns {Promise<object[]>} hits, best first, decorated with the search-level facts.
 * @throws {IndexError} when the index is closed, not ready, or the search was cancelled.
 */
async function search(state, options = {}) {
  if (state.closed) throw new IndexError('closed', 'the index is closed')
  if (!state.ready)
    throw new IndexError('index-not-ready', 'the index has not completed its first scan')
  ...
  if (signal !== null && signal.aborted === true)
    throw new IndexError('aborted', 'the search was cancelled before it started')
  const plan = planQuery(query)
  const windowSize = Math.min(MAX_CANDIDATES, Math.max(MIN_CANDIDATES, limit * CANDIDATE_FACTOR))

  const source =
    state.backend === 'sqlite'
      ? openSqliteSource(state, { plan, filters, windowSize })
      : openScanSource(state, { plan, filters, windowSize })

  const accepted = []
  let exhausted = false
  while (accepted.length < windowSize) {
    if (signal !== null && signal.aborted === true)
      throw new IndexError('aborted', 'the search was cancelled')
    const page = source.page()
    exhausted = page.done
    for (const candidate of page.rows) {
      // One match past the window is proof that more candidates exist.
      if (accepted.length >= windowSize) break
      const verified = await verifyCandidate(state, candidate, plan)
      if (verified === null) continue
      verified.backend = state.backend
      accepted.push(verified)
    }
    if (page.done || accepted.length >= windowSize) break
  }
  accepted.sort(compareHits)
  const hits = accepted.slice(0, limit)
  const truncation = source.truncated
    ? { truncated: true, reason: source.truncationReason }
    : exhausted
      ? { truncated: false, reason: null }
      : { truncated: true, reason: `candidate-window:${windowSize}` }
  const meta = {
    truncated: truncation.truncated,
    truncationReason: truncation.reason,
    candidateWindow: windowSize,
    rowsExamined: source.examined,
    candidatesScored: accepted.length,
  }
  state.lastSearch = { query, limit, backend: state.backend, ...meta }
  return withSearchMeta(hits, meta)
}

/**
 * Total order for hits: composite score, then the backend's own relevance, then path.
 *
 * @param {object} left - a scored hit.
 * @param {object} right - a scored hit.
...
/**
 * Compare two vault paths the way SQLite's BINARY collation does.
 *
 * SQLite orders TEXT by its UTF-8 bytes, which is code-point order; a plain JS
 * `left < right` orders by UTF-16 code units, and the two disagree as soon as a
 * path holds a non-BMP character (its surrogate pair sorts below U+E000..U+FFFF
 * in UTF-16 and above it in code-point order). Walking code point by code point
 * keeps the scan backend's prefix identical to the SQLite `ORDER BY n.path`
 * prefix, which is what makes the bounded substring window comparable.
 *
 * @param {string} left - a vault-relative path.
 * @param {string} right - a vault-relative path.
 * @returns {number} the comparison.
 */
...
/**
 * Attach the search-level facts to the result array without making them elements.
 *
 * The properties are non-enumerable, so the array still compares equal to a plain
 * list of hits while a caller can ask why the list is what it is: `truncated`,
 * `truncationReason`, `candidateWindow`, `rowsExamined` and `candidatesScored`.
 *
 * @param {object[]} hits - the sliced hits.
 * @param {{truncated: boolean, truncationReason: string|null, candidateWindow: number, rowsExamined: number, candidatesScored: number}} meta - the search facts.
 * @returns {object[]} the same array.
 */
function withSearchMeta(hits, meta) {
  Object.defineProperties(hits, {
    truncated: { value: meta.truncated, enumerable: false },
    truncationReason: { value: meta.truncationReason, enumerable: false },
    candidateWindow: { value: meta.candidateWindow, enumerable: false },
    rowsExamined: { value: meta.rowsExamined, enumerable: false },
    candidatesScored: { value: meta.candidatesScored, enumerable: false },
  })
  return hits
}

/**
 * A lazily paged candidate source, implemented identically by both backends.
 *
 * `page()` returns the next batch of *matching* candidates and `done` says the
 * source has nothing more to offer. `examined` counts the rows looked at (the
 * plan-mandated substring bound applies to exactly that number), and
 * `truncated`/`truncationReason` say a bound — not exhaustion — closed the
 * source, so a caller can never mistake "cut short" for "nothing there".
 *
 * @typedef {object} CandidateSource
 * @property {() => {rows: object[], done: boolean}} page
 * @property {number} examined
 * @property {boolean} truncated
 * @property {string|null} truncationReason
 */

/**
 * The SQLite candidate source for one query plan.
 *
 * @param {object} state - index state.
 * @param {{plan: object, filters: object, windowSize: number}} input - query plan, filters and window.
 * @returns {CandidateSource} the source.
 */
function openSqliteSource(state, { plan, filters, windowSize }) {
  return plan.branch === 'tokens'
    ? sqliteTokenSource(state, { plan, filters, windowSize })
    : sqliteSubstringSource(state, { plan, filters, windowSize })
}

/**
 * FTS5 candidates in `bm25 ASC` order, one row past the window to detect truncation.
 *
 * @param {object} state - index state.
 * @param {{plan: object, filters: object, windowSize: number}} input - query plan, filters and window.
 * @returns {CandidateSource} the source.
 */
function sqliteTokenSource(state, { plan, filters, windowSize }) {
  const where = []
  const params = []
  ...
  const rows =
    match === null
      ...
      : state.db
          .prepare(
            `
      SELECT ${NOTE_COLUMNS}, notes_fts.body AS body,
             bm25(notes_fts, ${BM25_WEIGHTS.join(', ')}) AS rank
      FROM notes_fts JOIN notes n ON n.rowid = notes_fts.rowid
      WHERE notes_fts MATCH ?${where.length > 0 ? ` AND ${where.join(' AND ')}` : ''}
      ORDER BY rank ASC, n.path ASC
      LIMIT ?
    `,
          )
          .all(match, ...params, windowSize + 1)
  const truncated = rows.length > windowSize
  const candidates = rows.slice(0, windowSize).map(toCandidate)
  let served = false
  return {
    examined: rows.length,
    truncated,
    truncationReason: truncated ? `candidate-window:${windowSize}` : null,
    page() {
      if (served) return { rows: [], done: true }
      served = true
      return { rows: candidates, done: true }
    },
  }
}

/**
 * The bounded substring branch: walk the *filtered* rows in path order.
 *
 * FTS5 cannot express a single CJK character, so the source reads rows itself.
 * Ordering by path keyset (`n.path > ? ORDER BY n.path ASC`, never `OFFSET`, and
 * code-point order, which is SQLite's BINARY order) makes the window a
 * deterministic, repeatable prefix — the same prefix the scan backend walks.
 *
 * The source owns the truth about its own completeness, and it enforces the
 * candidate window itself: it emits at most `windowSize` candidates, and the
 * moment a matching row beyond that window is found it sets `truncated` with
 * `candidate-window`, so a caller can never be told "complete" while a match was
 * withheld. Reaching the plan-mandated row bound sets `truncated` only once a
 * further row proves the bound — not the end of the vault — is the reason.
 *
 * @param {object} state - index state.
 * @param {{plan: object, filters: object, windowSize: number}} input - query plan, filters and the candidate window.
 * @returns {CandidateSource} the source.
 */
function sqliteSubstringSource(state, { plan, filters, windowSize }) {
  const needle = plan.needle.toLowerCase()
  const where = []
  const params = []
  applySqlFilters(filters, where, params)
  const filtersAnd = where.length > 0 ? `${where.join(' AND ')} AND ` : ''
  const pageSql = `
    SELECT ${NOTE_COLUMNS}, f.body AS body
    FROM notes n JOIN notes_fts f ON f.rowid = n.rowid
    WHERE ${filtersAnd}n.path > ?
    ORDER BY n.path ASC
    LIMIT ?
  `
  const probeSql = `SELECT n.path FROM notes n WHERE ${filtersAnd}n.path > ? ORDER BY n.path ASC LIMIT 1`
  const bound = state.scanBranchMaxRows
  const pageSize = state.scanBranchPageSize
  let lastPath = ''
  let examined = 0
  let emitted = 0
  let truncated = false
  ...
  return {
    ...
    page() {
      ...
      while (!done && !truncated) {
        const room = Math.min(pageSize, bound - examined)
        if (room <= 0) {
          if (state.db.prepare(probeSql).get(...params, lastPath) !== undefined) {
            truncated = true
            reason = `substring-scan-bound:${bound}`
          }
          done = true
          break
        }
        const batch = state.db.prepare(pageSql).all(...params, lastPath, room)
        if (batch.length === 0) {
          done = true
          break
        }
        ...
        if (batch.length < room) done = true
        for (const row of batch) {
          if (!`${row.title ?? ''}\n${row.body}`.toLowerCase().includes(needle)) continue
          if (emitted >= windowSize) {
            // A match past the window is proof the window withheld recall.
            truncated = true
            reason = `candidate-window:${windowSize}`
            done = true
            break
          }
      ...
      return { rows, done: done || truncated }
    },
  }
}

/**
 * The in-memory scan backend's candidate source for one query plan.
 *
 * It applies the same bound, in the same path order, and reports the same
 * truncation facts as the SQLite source, which is what keeps the two backends
 * filtering identically even when the bound cuts the scan.
 *
 * @param {object} state - index state.
 * @param {{plan: object, filters: object, windowSize: number}} input - query plan, filters and window.
 * @returns {CandidateSource} the source.
 */
function openScanSource(state, { plan, filters, windowSize }) {
  return plan.branch === 'tokens'
    ? scanTokenSource(state, { plan, filters, windowSize })
    : scanSubstringSource(state, { plan, filters, windowSize })
}

/**
 * In-memory candidates for a tokenised query, ranked by the proportion of tokens found.
 *
 * @param {object} state - index state.
 * @param {{plan: object, filters: object, windowSize: number}} input - query plan, filters and window.
 * @returns {CandidateSource} the source.
 */
function scanTokenSource(state, { plan, filters, windowSize }) {
  const candidates = []
  ...
  candidates.sort(
    (left, right) => right.matchedRatio - left.matchedRatio || comparePath(left.path, right.path),
  )
  const truncated = candidates.length > windowSize
  ...
  return {
    ...
    page() {
      if (served) return { rows: [], done: true }
      served = true
      return { rows: window, done: true }
    },
  }
}

/**
 * In-memory candidates for the bounded substring branch, walked in path order.
 *
 * It mirrors {@link sqliteSubstringSource} exactly — same predicate, same
 * code-point path order, same row bound, same candidate window, same truncation
 * reasons — which is what keeps the two backends filtering identically even when
 * a bound cuts the scan.
 *
 * @param {object} state - index state.
 * @param {{plan: object, filters: object, windowSize: number}} input - query plan, filters and the candidate window.
 * @returns {CandidateSource} the source.
 */
function scanSubstringSource(state, { plan, filters, windowSize }) {
  const needle = plan.needle.toLowerCase()
  const ordered = [...state.records.values()]
  ...
  return {
    ...
    page() {
      ...
      while (!done && !truncated) {
        ...
        const slice = ordered.slice(cursor, cursor + room)
        if (slice.length === 0) {
          done = true
          break
        }
        ...
        if (slice.length < room) done = true
        for (const record of slice) {
          if (!record.searchText.toLowerCase().includes(needle)) continue
          if (emitted >= windowSize) {
            truncated = true
            reason = `candidate-window:${windowSize}`
            done = true
          ...
          rows.push({ ...record, rank: null, matchedRatio: 0 })
          emitted += 1
        }
      }
      return { rows, done: done || truncated }
...
function applySqlFilters(filters, where, params) {
  ...
  if (Array.isArray(filters.excludeStatuses) && filters.excludeStatuses.length > 0) {
    where.push(
      `(n.status IS NULL OR n.status NOT IN (${filters.excludeStatuses.map(() => '?').join(', ')}))`,
    )
...
function matchesFilters(record, filters) {
  if (
    filters.kind === 'global' &&
    !(record.path === GLOBAL_USER_PATH || record.path.startsWith(METHOD_PREFIX))
  )
    return false
  if (
    filters.kind === 'project' &&
    !(
      record.project === filters.projectId ||
      (record.project === null && record.projectDirId8 === filters.projectId8)
    )
  )
    return false
  if (
    filters.filterProjectId !== undefined &&
    filters.filterProjectId !== null &&
    record.project !== filters.filterProjectId
  )
    return false
  if (
    Array.isArray(filters.excludeStatuses) &&
    record.status !== null &&
    filters.excludeStatuses.includes(record.status)
  )
    return false
  if (filters.type !== undefined && filters.type !== null && record.type !== filters.type)
    return false
  return true
}
...
async function verifyCandidate(state, candidate, plan) {
  ...
  try {
    ...
    throw new IndexError(
      'read-failed',
      `cannot stat ${candidate.path}: ${error.code ?? error.message}`,
      { cause: error },
    )
  }
...
async function waitReady(state, signal, timeoutMs = DEFAULT_READY_TIMEOUT_MS) {
  ...
  // The listener must not outlive the wait: a caller may reuse one signal for
  // many ready barriers, and a leaked listener per call would accumulate.
  clearTimeout(timer)
...
function status(state) {
  return {
    ...
    lastFullVerify:
      state.lastFullVerify === 0 ? null : new Date(state.lastFullVerify).toISOString(),
    lastSearch: state.lastSearch === null ? null : { ...state.lastSearch },
...
async function close(state) {
  ...
  if (db !== null) {
    try {
      ...
      /* an unusable handle has nothing to release */
    }
  }
}
