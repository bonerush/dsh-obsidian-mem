...
//     UTF-16 units, so the budget here is counted in UTF-8 bytes. 200 bytes
//     leaves room for a `.md` extension and a suffix.
//
...
/** The §6.4/R22 budget: the name must fit ext4's 255-byte limit with room to spare. */
export const MAX_BASENAME_BYTES = 200
/** Used when sanitizing removes everything the title had to offer. */
export const FALLBACK_BASENAME = 'untitled'
/**
 * Bytes a suffixed name reserves for the `.md` extension (R28).
 *
 * `MAX_BASENAME_BYTES` bounds the *basename*, and the emitted file name is that
 * basename plus `.md`. For a suffixed name the two must fit together, so
 * {@link withSuffix} cuts three bytes earlier. The unsuffixed stem keeps the
 * full budget that the Task 6 tests pin.
 */
const EXTENSION_BYTES = 3
 ...
 */
const EMOJI = /[\p{Extended_Pictographic}\p{Emoji_Presentation}\p{Regional_Indicator}\p{Emoji_Modifier}\u{FE0E}\u{FE0F}\u{200D}\u{20E3}]/gu
...
export function safeBasename(title, existingNames = []) {
  ...
  if (!Array.isArray(existingNames)) throw new RangeError('existingNames must be an array of strings')
  const taken = new Set()
 ...
 *
 * The whole emitted file name — suffixed basename plus `.md` — must stay inside
 * the §6.4/R22 budget (R28). The suffix and the extension are reserved first and
 * the tail (everything from the first dot) is kept only as far as what is left;
 * a pathological stem such as `a.` followed by 300 characters used to keep the
 * entire 199-byte tail and emit a 208-byte name. Whatever the input, the result
 * always carries the suffix — so it can never silently equal the stem it was
 * asked to disambiguate — and never exceeds the budget.
 *
...
function withSuffix(stem, suffix) {
  ...
  const budget = MAX_BASENAME_BYTES - EXTENSION_BYTES - Buffer.byteLength(suffixText, 'utf8')
