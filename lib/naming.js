 ...
 *   (excluding the note being written), compared case-folded and NFC-normalized.
...
function withSuffix(stem, suffix) {
  ...
  const budget = MAX_BASENAME_BYTES - Buffer.byteLength(`-${suffix}${tail}`, 'utf8')
  const kept = trimEdge(truncateToBytes(head, Math.max(0, budget)))
  return `${kept}-${suffix}${tail}`
}
