...
import { readRegistry, resolveBinding, resolveVaultRoot, isCloudManagedVaultRoot } from './vault.js'
...
/**
 * `mem_brief`'s status-only answer: an unbound working directory, or the P1
 * `not-ready-in-p1` marker. The P1 arm is kept because it is the documented shape
 * the Task 10 stub contract promised; the shipped service returns either the brief
 * value above or `unbound`, never the P1 marker.
 *
...
const BRIEF_OUTPUT = Object.freeze({
  oneOf: [BRIEF_VALUE_SCHEMA, briefStatusArm('unbound'), briefStatusArm('not-ready-in-p1')],
})
