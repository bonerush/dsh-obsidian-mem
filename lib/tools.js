...
export function createMemoryServices(options = {}) {
  ...
  async function autoBindProject(exec) {
    ...
    try {
      ...
      // it throws, and it stays false until the first skeleton file or the
      // registry row lands. A refusal before that — the §6.4 property preflight,
      // a registry whose recorded sha256 does not cover its body, a vault-root or
      // template refusal — leaves the repository exactly as the write found it,
      // so the pointer goes back and the next write retries the bind cleanly.
      // A failure that *did* create content keeps the identity (part of the
      // skeleton may exist); `mem_admin(action="bind", mode="local")` re-runs the
      // bootstrap, because the write path never does.
