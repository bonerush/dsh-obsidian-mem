...
export const TOOL_PARAMETERS = Object.freeze({
  mem_search: Object.freeze({
    ...
    scope: { type: 'string', enum: ['project', 'global', 'all'], default: 'project', description: 'project (default) is the bound project only; global is Methods/ plus _meta/user.md; all crosses projects.' },
  ...
  mem_admin: Object.freeze({
    ...
    path: { type: 'string', description: 'Only for promote: the vault-relative path of the source note a new Methods/ note is created from. The source is never moved or rewritten.' },
...
function searchTool(services) {
  return {
    name: 'mem_search',
    description: 'Search project memory. Defaults to the bound project; pass scope "global" for Methods/ and _meta/user.md, or "all" to cross projects.',
...
function adminTool(services) {
  return {
    name: 'mem_admin',
    description: 'Low-frequency vault maintenance: lint (read-only unless report=true and/or prune=true), index status/rebuild, bind (show/local/fork/retain), the project list, promote a note into Methods/, and the pending job queue with an explicit retry.',
