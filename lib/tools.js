...
const ADMIN_ACTION_PARAMETERS = Object.freeze({
  // `report` is the explicit request the design reserves for the writing half of
  // a lint: without it the action only inspects.
  lint: Object.freeze(['report']),
...
export const TOOL_PARAMETERS = Object.freeze({
  ...
  mem_admin: Object.freeze({
    ...
    report: { type: 'boolean', default: false, description: 'Only for lint: additionally write the dated _meta/Lint Report note and apply the .history retention policy. Default false = read-only.' },
  }),
})
...
function adminTool(services) {
  return {
    name: 'mem_admin',
    description: 'Low-frequency vault maintenance: lint (read-only unless report=true), index status/rebuild, bind (show/local/fork/retain), the project list, promote a note into 方法/, and the pending job queue with an explicit retry.',
...
function forwardAdminArguments(args) {
  ...
  if (args.action === 'lint') forwarded.report = args.report ?? false
  return forwarded
}
