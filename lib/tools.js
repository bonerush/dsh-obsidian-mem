...
export const TOOL_PARAMETERS = Object.freeze({
  ...
  mem_admin: Object.freeze({
    action: {
      type: 'string',
      required: true,
      enum: ['lint', 'index', 'bind', 'projects', 'promote', 'jobs'],
...
const TOOL_ALLOWED_KEYS = Object.freeze(
  Object.fromEntries(
...
export function createMemoryServices(options = {}) {
  ...
  return {
    ...
    async admin(args, signal, exec) {
      ...
      if (args.action === 'bind')
        return bindAction(args, { config, dataRoot, home, cwd, exec, onBound: rememberBinding })
      if (args.action === 'lint') {
