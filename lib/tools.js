...
import { bootstrapVault, readRegistry, releaseCreatedPointer, resolveBinding, resolveVaultRoot, isCloudManagedVaultRoot } from './vault.js'
...
/**
 * Bootstrap refusals that happen before the first vault write (spec §6.4/R18).
 *
 * `bootstrapVault` runs the property preflight before `prepareRegistry`, so these
 * two codes mean "the vault was not touched" and a pointer minted by this call
 * can be handed back safely. Every later refusal may already have written the
 * registry row, so the identity stays and the error is reported as it is.
 */
const PREFLIGHT_BOOTSTRAP_REFUSALS = Object.freeze(new Set(['property-type-conflict', 'property-preflight-conflict']))
...
function writeTool(services) {
  return {
    name: 'mem_write',
    description: 'Write a memory note. Without an id this always CREATES a note with a fresh random id; pass an existing id to update it. Superseding verifies the old id. A Git repository with no .obsidian-mem pointer is bound on first write.',
...
function logTool(services) {
  return {
    name: 'mem_log',
    description: 'Append one idempotent log entry to the day log, or with section "hot" to the controlled hot zone. A Git repository with no .obsidian-mem pointer is bound on first write.',
...
export function createMemoryServices(options = {}) {
   ...
   * and `mode: 'local'` reuses whatever identity it finds.
   *
   * @param {object} exec - the tool execution context.
  ...
  async function autoBindProject(exec) {
    const key = cwdOf(exec) ?? cwd
    const probe = await resolveBinding({ cwd: key, vaultRoot: config.vaultPath, mode: 'show', home })
    if (probe.kind !== 'unbound' || probe.reason !== 'no-pointer') return { binding: null, refusal: probe }
    const resolution = await resolveBinding({ cwd: key, vaultRoot: config.vaultPath, mode: 'local', dataRoot, home })
    if (resolution.kind !== 'bound') return { binding: null, refusal: resolution }
    try {
      await bootstrapVault(resolution, { initGitOnCreate: config.initGitOnCreate === true, home, dataRoot })
    } catch (error) {
      // The property preflight (spec §6.4/R18) refuses before the first vault
      // write, and its refusal must not leave the identity it just minted
      // behind: the next write would resolve as bound and skip the preflight
      // that refused it. Failures after the registry write keep the identity —
      // the vault already knows it — and are reported as they are.
      if (PREFLIGHT_BOOTSTRAP_REFUSALS.has(error?.code)) await releaseCreatedPointer(resolution)
      throw error
    }
  ...
  return {
    ...
    async admin(args, signal, exec) {
      ...
      if (args.action === 'bind') return bindAction(args, { config, dataRoot, home, cwd, exec, onBound: rememberBinding })
      if (args.action === 'lint') {
 ...
 */
async function bindAction(args, { config, dataRoot, home, cwd, exec, onBound }) {
  ...
  // directory: `resolveProject` memoizes a miss, and without this the six tools
  // would keep replaying it until the plugin is reloaded.
