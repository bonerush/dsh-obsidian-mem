...
//   * **P1 never fakes a result.** `mem_brief` returns a structured
//     `not-ready-in-p1` value until Task 11 binds it, and `mem_admin` exposes
//     only what exists today: the index status, the vault's project list, and
//     `bind(mode="show")`. `lint`, `promote`, `jobs` and the identity-changing
//     bind modes answer `not-ready-in-p1` instead of pretending. Results carry
//     vault-relative paths; no `obsidian://` URI is invented for a vault this
//     plugin cannot prove Obsidian has opened.
...
import { DEFAULT_LIMIT, openIndex } from './index-db.js'
import { appendLog, MemoryError, writeMemory } from './memory.js'
import { updateHot } from './hot.js'
import { normalizeType } from './routing.js'
import { readNote, searchNotes } from './search.js'
import { readRegistry, resolveBinding, resolveVaultRoot } from './vault.js'
...
/** What `mem_brief` answers until Task 11 binds it to a real brief. */
const BRIEF_NOT_READY = Object.freeze({
  status: 'not-ready-in-p1',
  message: 'mem_brief is bound in Task 11: the budgeted recall brief is not composed yet, so there is nothing to report',
})
...
export const TOOL_PARAMETERS = Object.freeze({
  ...
  mem_admin: Object.freeze({
    ...
    path: { type: 'string', description: 'Only for lint/promote: the note a lint report or a promoted method note is about.' },
    rebuild: { type: 'boolean', default: false, description: 'Only for index: re-hash every source instead of the cheap mtime/size pass.' },
    mode: { type: 'string', enum: ['show', 'local', 'fork', 'retain'], default: 'show', description: 'Only for bind. show reports without writing; the other modes change project identity and are not available in P1.' },
    jobId: { type: 'string', description: 'Only for jobs: the failed job to inspect.' },
    retry: { type: 'boolean', default: false, description: 'Only for jobs: explicitly retry the named job.' },
  }),
})
...
/** The P1 `mem_brief` value; Task 11 replaces this schema with the real brief. */
const BRIEF_OUTPUT = Object.freeze({
  type: 'object',
  additionalProperties: false,
  properties: {
    status: { type: 'string', required: true, const: 'not-ready-in-p1' },
    message: { type: 'string', required: true },
  },
})
...
const INDEX_STATUS_SCHEMA = Object.freeze({
  ...
  properties: {
    ...
    dbPath: { ...STRING_OR_NULL, required: true },
...
/** `bind(mode="show")`: the read-only resolution, reported as it came back. */
const BIND_SHOWN_RESULT = Object.freeze({
  type: 'object',
  additionalProperties: false,
  properties: {
    mode: { type: 'string', required: true, const: 'show' },
    status: { type: 'string', required: true, const: 'shown' },
    resolution: { ...OPEN_OBJECT, required: true },
  },
})

/** `bind(mode="local"|"fork"|"retain")`: an identity change P1 does not perform. */
const BIND_NOT_READY_RESULT = Object.freeze({
  type: 'object',
  additionalProperties: false,
  properties: {
    mode: { type: 'string', required: true, enum: ['local', 'fork', 'retain'] },
    status: { type: 'string', required: true, const: 'not-ready-in-p1' },
    message: { type: 'string', required: true },
  },
})

const NOT_READY_RESULT = Object.freeze({
  type: 'object',
  additionalProperties: false,
  properties: {
    status: { type: 'string', required: true, const: 'not-ready-in-p1' },
...
const ADMIN_OUTPUT = Object.freeze({
  oneOf: [
    ...
    adminArm('bind', { oneOf: [BIND_SHOWN_RESULT, BIND_NOT_READY_RESULT] }),
    adminArm('lint', NOT_READY_RESULT),
    adminArm('promote', NOT_READY_RESULT),
    adminArm('jobs', NOT_READY_RESULT),
  ],
})
...
function adminTool(services) {
  return {
    name: 'mem_admin',
    description: 'Low-frequency vault maintenance: index status, project list, and a read-only bind report.',
...
export function createMemoryServices(options = {}) {
  ...
  const vaultRoot = () => {
    if (vaultPromise === null) {
      vaultPromise = resolveVaultRoot(config.vaultPath, { home }).then((vault) => vault.root)
   ...
   * @throws {MemoryError} with code `not-bound`.
   */
  ...
  return {
    ...
    async admin(args, signal, exec) {
      ...
      if (args.action === 'bind') {
        ...
        if (mode !== 'show') {
          return {
            ...
            result: {
              mode,
              status: 'not-ready-in-p1',
              message: `bind mode "${mode}" changes project identity and is not implemented in P1; "show" reports the current binding without writing`,
            },
          }
        }
        ...
        return { action: 'bind', result: { mode: 'show', status: 'shown', resolution: toResolutionView(resolution) } }
      }
      return { action: args.action, result: notReadyResult(args.action) }
    },
...
/** The structured P1 boundary marker for an action Task 17 will implement. */
function notReadyResult(action) {
  return {
    status: 'not-ready-in-p1',
    message: `mem_admin(action="${action}") is bound in Task 17 and is not implemented in P1; no ${action} result is produced or faked`,
  }
}
