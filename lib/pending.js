...
async function writeAtomicIn(directory, fileName, value, { beforeRename } = {}) {
  ...
  const temporary = join(directory, `.${fileName}.${process.pid}.${Date.now()}.${Math.random().toString(16).slice(2)}.tmp`)
  let handle = null
  try {
 ...
 */
export async function persistJobOutput(jobId, output, { queueRoot = defaultQueueRoot(), beforeRename } = {}) {
  const fileName = jobFileName(jobId)
  ...
  return withLock(`job:${path}`, async () => {
    ...
    const next = { ...existing, state: normalized.state, output: normalized, updatedAt: new Date().toISOString() }
    await writeJobAtomic(queueRoot, next, { beforeRename })
    return normalizeJob(next)
  })
}
...
export function normalizeJobOutput(output) {
  ...
  if (!OUTPUT_STATES.includes(output.state)) {
    throw new PendingError('output-invalid', `a persisted model output state must be one of ${OUTPUT_STATES.join(', ')}`)
  }
  if (typeof output.raw !== 'string') {
    throw new PendingError('output-invalid', 'a persisted model output must carry the raw model text')
  }
  ...
  if (output.usage !== undefined && output.usage !== null && (typeof output.usage !== 'object' || Array.isArray(output.usage))) {
...
export function normalizeRefusals(value, limit = MAX_REFUSED_ENTRIES) {
  ...
  return value.slice(0, limit).map((entry) => {
    const source = entry !== null && typeof entry === 'object' && !Array.isArray(entry) ? entry : { reason: entry }
    const refusal = {
...
export async function writeResultReceipt(queueRoot, receipt) {
  ...
  await writeAtomicIn(resultReceiptDir(queueRoot, normalized.projectId), `${normalized.jobId}.json`, normalized)
  return normalized
}
...
export async function readResultReceiptFiles(queueRoot, projectId) {
  ...
  for (const name of [...names].sort()) {
    ...
    try {
      ...
      invalid.push({ file: `${RECEIPTS_DIRNAME}/${projectId}/${name}`, reason: error.code ?? 'receipt-invalid' })
    }
  }
...
function normalizeResultReceipt(receipt) {
  ...
  if (!isSafeJobId(receipt.jobId)) {
    throw new PendingError('receipt-invalid', `a result receipt must name a safe job id: ${JSON.stringify(receipt.jobId)}`)
  }
  ...
  if (!Number.isSafeInteger(receipt.toSeq) || receipt.toSeq < 0) {
    throw new PendingError('receipt-invalid', 'a result receipt must carry a non-negative integer toSeq')
  }
  if (!RESULT_KINDS.includes(receipt.result)) {
    throw new PendingError('receipt-invalid', `a result receipt result must be one of ${RESULT_KINDS.join(', ')}`)
  }
  ...
  return JSON.parse(JSON.stringify({
    ...
    attempts: Number.isSafeInteger(receipt.attempts) && receipt.attempts >= 0 ? receipt.attempts : 0,
    ...
    // count so a truncated list is never mistaken for the whole story.
    refused,
    refusedCount: Number.isSafeInteger(receipt.refusedCount) && receipt.refusedCount >= refused.length ? receipt.refusedCount : refused.length,
    index: typeof receipt.index === 'string' && receipt.index !== '' ? receipt.index : 'none',
    usage: receipt.usage === undefined || receipt.usage === null || typeof receipt.usage !== 'object' ? null : receipt.usage,
...
export async function readProcessedRecords(queueRoot) {
  ...
  for (const name of [...names].sort()) {
    ...
    try {
      ...
      if (existing === undefined || record.ranges.length >= existing.ranges.length) records.set(record.sessionId, record)
    } catch (error) {
      invalid.push({ file: `${PROCESSED_DIRNAME}/${name}`, reason: error.code ?? 'processed-invalid' })
    }
  }
 ...
 */
export async function recordProcessedRange(queueRoot, { sessionId, fromSeq, toSeq, jobId = null, at = new Date().toISOString() }) {
  if (!Number.isSafeInteger(fromSeq) || !Number.isSafeInteger(toSeq) || fromSeq < 0 || toSeq < fromSeq) {
  ...
  return withLock(`record:${path}`, async () => {
    ...
    if (existing !== null && existing.ranges.some(([from, to]) => from <= fromSeq && to >= toSeq)) return existing
...
function normalizeProcessed(record) {
  ...
  if (!Array.isArray(record.ranges)) throw new PendingError('processed-invalid', 'a processed record must carry ranges')
  const ranges = []
