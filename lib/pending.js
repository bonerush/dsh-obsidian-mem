...
/**
 * The stored `lastError` as the one string the `jobs` result declares.
 *
 * The queue keeps an object — `{code, message, at}` — because the reason, the
 * line and the time are separately useful to a reader of the job file, while the
 * tool schema declares a single string. This is the one place the two shapes
 * meet. A stored `message` is `describeError`'s output and already begins with
 * its code, so the code is prepended only when it is missing: the naive join
 * prints `truncated: truncated: …`.
 *
 * @param {unknown} lastError - the stored value: an object, a legacy string, or null.
 * @returns {string|null} one printable line, or null when there is nothing to report.
 */
export function normalizeJobError(lastError) {
  if (typeof lastError === 'string') return lastError === '' ? null : lastError
  if (lastError === null || typeof lastError !== 'object') return null
  const code = typeof lastError.code === 'string' ? lastError.code : ''
  const message = typeof lastError.message === 'string' ? lastError.message : ''
  if (message === '') return code === '' ? null : code
