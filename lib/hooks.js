 ...
 */
export function registerHooks(ctx, { resolveBinding, index, buildBrief, config }) {
  ...
  const disposePreStep = ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
    ...
    let plan = null
    try {
      plan = await planInjection(deps, bindings, id, agent, state, signal)
    } catch {
      // Fail open. A recall that throws must never break the turn: the model
      // simply gets no brief this step, and the next step tries again.
      plan = null
    }
    if (plan === null || isAborted(signal)) return decision
 ...
 */
async function planInjection(deps, bindings, id, agent, state, signal) {
  const binding = await boundBinding(deps, bindings, id, agent)
  if (binding === null || isAborted(signal)) return null
  const handle = await deps.index({ agent, signal })
