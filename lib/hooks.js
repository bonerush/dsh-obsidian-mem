...
export function registerHooks(
  ...
  const deps = { resolveBinding, index, buildBrief, config, lintHint, diagnostics }
  ...
  const disposePreStep = ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
    ...
    const cycle = (async () => {
      ...
      if (plan === null && hint === null) {
        recordDiagnostic(diagnostics, 'brief', { outcome: 'none' })
        return null
      }
      recordDiagnostic(diagnostics, 'brief', { outcome: plan === null ? 'hint-only' : 'injected' })
