 ...
 * @returns {Promise<{text: string, charCount: number, hotHash: string|null, hotItems: object[], indexState: object}>} the brief.
...
export async function buildBrief(binding, options = {}) {
  ...
  if (mode === 'delta') {
    ...
    return { text: composed.text, charCount: composed.charCount, hotHash: hot.hash, hotItems: hot.items, indexState }
  }
 ...
 * @returns {{text: string, charCount: number, omitted: number}} the brief text.
 ...
 * @param {string[]} input.extras - plugin notices that are not vault data.
 * @param {Function} input.footer - `(used, omitted) => string`.
 * @returns {{text: string, charCount: number, omitted: number}} the rendered brief.
 */
function renderBrief({ sections, count, preamble, extras, footer }) {
  ...
  const notes = [...extras]
  if (omitted > 0) notes.push(`（简报预算已满，省略 ${omitted} 条完整条目。）`)
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    if (next === used) return { text, charCount: next, omitted }
    used = next
  }
  const text = `${content}\n\n${footer(used, omitted)}`
  return { text, charCount: charCount(text), omitted }
}
