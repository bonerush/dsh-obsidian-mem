...
// therefore sharing `_meta/项目注册表.md` and `_meta/log.md` — serialize, and a
...
const DEFAULT_IO = Object.freeze({ lstat: fs.lstat, readFile: fs.readFile })
/** Error codes that mean "this read failed", never "the file is empty". */
const TRANSIENT_READ_CODES = Object.freeze([
  'EDEADLK', 'EBUSY', 'EAGAIN', 'EIO', 'ETIMEDOUT', 'EINTR', 'ENOTCONN', 'EHOSTDOWN', 'ESTALE',
])
...
const FAULT_NAMES = Object.freeze(['new-note', 'old-status', 'moc', 'publish', 'receipt', 'receipt-store', 'index-notify'])
 ...
 * @param {{dataRoot?: string, failAfter?: string, notifyIndex?: Function, lockTimeoutMs?: number, pollMs?: number, io?: {lstat?: Function, readFile?: Function}, home?: string}} [options] - data root (defaults to `resolveDataRoot()`), fault injection, index notifier, lock tuning and a target-read seam.
...
export async function runTransaction(binding, request, options = {}) {
  ...
  if (typeof dataRoot !== 'string' || dataRoot.length === 0) throw new RangeError('dataRoot must be a non-blank path')
  if (notifyIndex !== undefined && notifyIndex !== null && typeof notifyIndex !== 'function') {
    throw new RangeError('notifyIndex must be a function')
  }
  if (!Number.isInteger(lockTimeoutMs) || lockTimeoutMs < 0) throw new RangeError('lockTimeoutMs must be a non-negative integer')
  if (!Number.isInteger(pollMs) || pollMs <= 0) throw new RangeError('pollMs must be a positive integer')
  ...
  const lock = await acquireLock({ identity, dataRoot, txId: plan.txId, lockTimeoutMs, pollMs })
  ...
  try {
    ...
    if (plan.idempotencyKey !== null) {
      const stored = await readReceiptRecord(receiptStorePath(dataRoot, identity.vaultHash, identity.projectId, plan.idempotencyKey))
    ...
    if ((await readManifestRecord(manifestPath(dataRoot, identity.vaultHash, plan.txId))) !== null) {
      throw new TransactionError('txid-in-use', `transaction ${plan.txId} already has a manifest for this vault`)
    }
    ...
    manifest.receipt.result.index = await settleIndexNotification(manifest, identity, dataRoot, notifyIndex)
...
async function applyTargets(manifest, identity, dataRoot, { io, fault }) {
  ...
  await writeManifest(manifest, dataRoot)
  await pruneLogSnapshot(manifest, identity)
  if (fault.afterReceiptCommit) throw new InjectedCrash('receipt-store')
...
function normalizeRequest(request) {
  ...
  const { txId } = request
  if (typeof txId !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(txId)) {
    throw new TransactionError('invalid-tx-id', `txId must match [A-Za-z0-9][A-Za-z0-9._-]{0,127}: ${JSON.stringify(txId)}`)
  }
  ...
  if (idempotencyKey !== null && (typeof idempotencyKey !== 'string' || idempotencyKey.length === 0)) {
    throw new TransactionError('invalid-request', 'idempotencyKey must be a non-blank string when present')
  }
  ...
  for (const entry of [...creates, ...updates]) {
    ...
    if (typeof path !== 'string' || path.length === 0) {
      throw new TransactionError('invalid-request', 'every create and update needs a non-blank vault-relative path')
    }
    if (seen.has(path)) throw new TransactionError('duplicate-target', `${path} appears more than once in one transaction`)
    seen.add(path)
  }
  if (receipt !== null && seen.has(LOG_RELATIVE_PATH)) {
    throw new TransactionError('duplicate-target', `${LOG_RELATIVE_PATH} is written by the receipt and cannot also be a target`)
  }
...
function normalizeFault(failAfter, plan) {
  ...
  if (failAfter === 'new-note') {
    if (plan.creates.length === 0) throw new RangeError('failAfter "new-note" needs at least one create')
    fault.afterTarget.set(0, 'new-note')
  } else if (failAfter === 'old-status') {
    if (plan.updates.length === 0) throw new RangeError('failAfter "old-status" needs at least one update')
...
async function buildManifest(identity, plan, dataRoot, { io }) {
  ...
  for (const [index, entry] of plan.creates.entries()) {
    ...
    if ((await lstatOrNull(absolute)) !== null) {
      throw new TransactionError('target-exists', `${entry.path} already exists; a create never overwrites`)
    }
  ...
  for (const [offset, entry] of plan.updates.entries()) {
    ...
    if (entry.transform !== undefined && typeof entry.transform !== 'function') {
      throw new TransactionError('invalid-request', `updates[${offset}].transform must be a function`)
    }
    if (entry.transform === undefined && entry.contents === undefined) {
      throw new TransactionError('missing-expected-hash', `updates[${offset}] for ${entry.path} needs contents or a transform`)
    }
    if (entry.transform === undefined && entry.hash !== null && !/^[0-9a-f]{64}$/.test(String(entry.hash ?? ''))) {
      throw new TransactionError('missing-expected-hash', `updates[${offset}] for ${entry.path} needs an expected sha256 hash`)
    }
    const absolute = await resolveVaultFile(identity.vaultRoot, entry.path)
    ...
    if (entry.transform !== undefined) {
      if (current !== null && entry.hash === null) {
        throw new TransactionError('target-exists', `${entry.path} exists, but the transaction requires it to be created`)
      }
      if (current === null && typeof entry.hash === 'string' && entry.hash !== '*') {
        throw new TransactionError('hash-mismatch', `${entry.path} is missing (expected ${entry.hash})`)
      }
      if (current !== null && typeof entry.hash === 'string' && entry.hash !== '*' && hashBytes(current) !== entry.hash) {
        throw new TransactionError('hash-mismatch', `${entry.path} changed after it was read (expected ${entry.hash}, found ${hashBytes(current)})`)
      }
      const produced = await entry.transform(current === null ? null : Buffer.from(current))
      afterBytes = produced === null || produced === undefined ? null : toBytes(produced, `updates[${offset}].transform()`)
    } else {
      const observed = hashBytes(current)
      if (observed !== entry.hash) {
        throw new TransactionError('hash-mismatch', `${entry.path} changed after it was read (expected ${entry.hash}, found ${observed})`)
      }
      afterBytes = toBytes(entry.contents, `updates[${offset}]`)
    }

    const mode = afterBytes === null ? 'noop' : current === null ? 'create' : afterBytes.equals(current) ? 'noop' : 'update'
    targets.push(makeTarget({
      index,
      txId: plan.txId,
      path: entry.path,
...
function makeTarget({ index, txId, path, mode, beforeHash, beforeBytes, afterBytes }) {
  ...
  return {
    ...
    snapshotPath: mode === 'update' ? joinRelative(HISTORY_ROOT, `${txId}/${index}__${file}`) : null,
    state: 'pending',
    observedHash: null,
    reason: null,
    beforeBytes,
...
async function publish(manifest, target, identity, { io }) {
  ...
  if (target.mode === 'create') {
    if ((await lstatOrNull(absolute)) !== null) {
      throw new TransactionError('target-exists', `${target.path} appeared while transaction ${manifest.txId} was running`)
    }
  ...
  if (hashBytes(current) !== target.beforeHash) {
    throw new TransactionError('target-changed', `${target.path} changed after it was read; refusing to overwrite the newer revision`)
  }
...
async function rollback(manifest, identity, dataRoot, { io, reason = 'failed' }) {
  ...
  for (const target of [...manifest.targets].reverse()) {
    ...
    const alreadyUndone = target.mode === 'create' ? current === null : currentHash === target.beforeHash
    if (alreadyUndone) {
...
async function discard(manifest, identity, dataRoot) {
  await cleanupStaging(manifest, identity)
  await fs.rm(join(identity.vaultRoot, HISTORY_ROOT, manifest.txId), { recursive: true, force: true }).catch(() => {})
  await removeManifest(manifest, dataRoot)
}
 ...
 * @param {{dataRoot?: string, notifyIndex?: Function, io?: object, home?: string, lockTimeoutMs?: number, pollMs?: number}} [options] - data root, notifier, read seam and lock tuning.
  ...
  for (const name of await listManifestNames(directory)) {
    ...
    try {
      if (manifest.schema !== TRANSACTION_SCHEMA) {
        throw new TransactionError('manifest-schema', `transaction ${manifest.txId} was written by schema ${JSON.stringify(manifest.schema)}; this plugin understands ${TRANSACTION_SCHEMA} only`)
      }
      ...
      if (outcome.conflicts.length > 0) {
        report.unresolved.push(unresolvedEntry(manifest, 'external-edit', outcome.conflicts.map((target) => target.path)))
      ...
      report.unresolved.push({
        ...
        paths: manifest.targets.filter((target) => target.state === 'written').map((target) => target.path),
      })
    }
  }
  return report
...
/**
 * Deliver (or keep queued) the post-commit index notification.
 *
 * An index failure never undoes the vault write (spec §10.4.4): it leaves the
 * manifest in place as the durable queue and records the reason.
 *
...
async function settleIndexNotification(manifest, identity, dataRoot, notifyIndex) {
  if (manifest.steps.indexNotified) {
    // The notification was already accepted, so only the manifest is left over.
    await removeManifest(manifest, dataRoot)
    return 'notified'
  }
  if (typeof notifyIndex !== 'function') {
    return manifest.receipt.result.index === 'stale' ? 'stale' : 'queued'
  }
  try {
...
async function storeReceipt(manifest, identity, dataRoot) {
  await writeReceiptRecord(
    receiptStorePath(dataRoot, identity.vaultHash, identity.projectId, manifest.idempotencyKey ?? manifest.txId),
    manifest.receipt,
  )
}
...
export async function markIndexNotified(binding, options = {}) {
  ...
  if (manifest === null || !manifest.steps?.receipted) return false
...
async function writeManifest(manifest, dataRoot) {
  ...
  await writeJsonAtomic(manifestPath(dataRoot, manifest.vaultHash, manifest.txId), serializeManifest(manifest))
}
 ...
 * @throws {TransactionError} when the manifest exists but cannot be parsed.
 */
async function readManifestRecord(path) {
  ...
  try {
    ...
    throw new TransactionError('manifest-unreadable', `cannot read the transaction manifest at ${path}`, { cause: error })
  }
  try {
    ...
    throw new TransactionError('manifest-corrupt', `the transaction manifest at ${path} is not valid JSON`, { cause: error })
  }
}
...
async function writeHistoryFile(identity, manifest, target, label, bytes) {
  const relative = joinRelative(HISTORY_ROOT, `${manifest.txId}/${target.index}__${label}__${basename(target.path)}`)
...
async function historyDestination(identity, txId, target, label) {
  const relative = joinRelative(HISTORY_ROOT, `${txId}/${target.index}__${label}__${basename(target.path)}`)
  const absolute = await resolveVaultFile(identity.vaultRoot, relative)
  await fs.mkdir(dirname(absolute), { recursive: true })
  return absolute
}
...
async function writeFailureRecord(manifest, identity, dataRoot, conflicts, reason) {
  ...
  await writeJsonAtomic(failureRecordPath(dataRoot, identity.vaultHash, identity.projectId, manifest.txId), record)
  const absolute = await resolveVaultFile(identity.vaultRoot, joinRelative(HISTORY_ROOT, `${manifest.txId}/manifest.json`))
  await fs.mkdir(dirname(absolute), { recursive: true })
 ...
 *
 * @param {{identity: object, dataRoot: string, txId: string, lockTimeoutMs: number, pollMs: number}} input - lock inputs.
 ...
 */
async function acquireLock({ identity, dataRoot, txId, lockTimeoutMs, pollMs }) {
  ...
  for (;;) {
    ...
    try {
      ...
      // Register before the first await: a second transaction in this process
      // must never mistake this lock for one an earlier crash abandoned.
    ...
    if (held !== null) {
      if (held.corrupt) {
        corrupt = true
      } else if (isStaleLock(held.record, path)) {
    ...
    if (Date.now() >= deadline) {
      if (corrupt) {
        throw new TransactionError('lock-corrupt', `the vault lock at ${path} is unreadable; inspect and remove it by hand`)
      }
...
async function readLockRecord(path) {
  ...
  try {
    ...
    if (record === null || typeof record !== 'object' || Array.isArray(record) || typeof record.token !== 'string') {
...
async function assertParentDirectory(vaultRoot, relativePath) {
  ...
  if (info === null) {
    throw new TransactionError('missing-parent-directory', `${relativePath} has no parent directory in the vault`)
  }
  if (!info.isDirectory() || info.isSymbolicLink()) {
    throw new TransactionError('missing-parent-directory', `the parent of ${relativePath} is not a directory`)
  }
}
...
function assertOwnership(entry, bytes, path) {
  ...
  if (!hasFrontmatter) {
    if (entry.own?.id !== undefined) {
      throw new TransactionError('ownership-unproven', `${path} has no frontmatter, so it cannot be the note ${entry.own.id} claims to be`)
    }
    return
  }
  let note
  try {
    ...
    throw new TransactionError('ownership-unproven', `${path} has frontmatter this plugin cannot parse, so ownership cannot be proven`, { cause: error })
  }
  const data = note.data ?? {}
  if (data.trust === 'owner') {
    throw new TransactionError('human-owned', `${path} declares trust:owner; the plugin never rewrites a human-owned note`)
  }
  if (entry.own?.id !== undefined && data.id !== entry.own.id) {
    throw new TransactionError('ownership-mismatch', `${path} carries id ${JSON.stringify(data.id)}, not ${JSON.stringify(entry.own.id)}`)
  }
}
...
async function readGuarded(absolute, relative, { io, mustExist = true }) {
  ...
  if (!info.isFile()) throw new TransactionError('target-not-a-file', `${relative} is not a regular file`)
  ...
  if (bytes.length === 0 && info.size > 0) {
    throw new TransactionError('short-read', `${relative} reported ${info.size} bytes but read back empty; refusing to overwrite it`)
  }
  return bytes
}
...
function toBytes(value, label) {
  ...
  throw new TransactionError('invalid-request', `${label}.contents must be a string, Buffer or Uint8Array`)
}
