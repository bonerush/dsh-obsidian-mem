...
export async function runTransaction(binding, request, options = {}) {
  ...
  try {
    ...
    if ((await lstatOrNull(join(identity.vaultRoot, HISTORY_ROOT, plan.txId))) !== null) {
      throw new TransactionError('txid-in-use', `transaction ${plan.txId} already left history in ${HISTORY_ROOT}/${plan.txId}`)
    }
...
async function rollback(manifest, identity, dataRoot, { io, reason = 'failed' }) {
  ...
  for (const target of [...manifest.targets].reverse()) {
    ...
    const alreadyUndone = failure === null
  ...
  for (const name of await listManifestNames(directory)) {
    ...
    try {
      ...
      if (outcome.conflicts.length > 0) {
        ...
        report.unresolved.push(unresolvedEntry(manifest, reason, outcome.conflicts.map((target) => target.path)))
...
export async function markIndexNotified(binding, options = {}) {
  ...
  if (manifest === null || !manifest.steps?.receipted || manifest.steps?.stored !== true) return false
...
async function acquireLock({ identity, dataRoot, txId, lockTimeoutMs, pollMs }) {
  ...
  for (let attempt = 0; ; attempt += 1) {
    ...
    try {
      ...
      // Registered immediately after the link resolves: a read that could see
      // this file was necessarily issued after the link, so its callback runs
      // after this line and it can never mistake our live lock for an abandoned
      // one.
