...
const LINK_UNSUPPORTED_CODES = Object.freeze(['EPERM', 'EACCES', 'EOPNOTSUPP', 'ENOTSUP', 'EXDEV', 'ENOSYS'])
 ...
 * @param {function({identity: object, dataRoot: string}): Promise<*>} task - the lock-scoped work.
...
export async function withVaultLock(binding, task, options = {}) {
  ...
  const identity = await vaultIdentity(binding, { home })
  const lock = await acquireLock({ identity, dataRoot, txId: 'vault-lock', lockTimeoutMs, pollMs, io })
  try {
    if (recover === true) {
