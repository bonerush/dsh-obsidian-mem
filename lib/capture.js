...
import { PENDING_SCHEMA, readPendingJobs, writeJobAtomic } from './pending.js'
...
const CREDENTIAL_PATTERNS = Object.freeze([
  ...
  // A PEM private-key block, whatever its algorithm.
 ...
 *
 * Returns `null` for anything that is not a completed, real-user root turn, or
 * whose projection is empty after scrubbing. Returns the persisted job both for
 * a fresh write and for an idempotent repeat (the same turn reaching the queue
 * twice — notification plus flush back-fill — is one job, not two).
 *
...
/**
 * Queue recovery: read every committed job of a queue.
 *
...
export async function recoverPending({ queueRoot, warn } = {}) {
  ...
  let result
  try {
    result = await readPendingJobs(queueRoot)
  } catch (error) {
  ...
  if (result.invalid.length > 0) {
    warnLine(
      warn,
      `obsidian-mem: ${result.invalid.length} pending job file(s) could not be read and were skipped: ${result.invalid
        .map((entry) => `${entry.file} (${entry.reason})`)
        .join(', ')}`,
    )
  }
  return { jobs: result.jobs, recovered: result.jobs.length, invalid: result.invalid }
}
...
export function createCapture({ queueRoot, config, resolveBinding, warn, now } = {}) {
   ...
   *
   * Never a turn-end signal by itself — the scan is the signal.
   */
  async function flush(session) {
    ...
    for (const event of eventsOf(session)) {
      ...
      if (isCovered(known, id, seq)) continue
      scanned.push(claimCapture(claims, session, event))
    }
    if (scanned.length > 0) await Promise.allSettled(scanned)
  }
...
function completedTurnEvents(events) {
  ...
  for (const event of events) {
    ...
    const start = starts.has(event.data.turn) ? starts.get(event.data.turn) : 0
...
/** Whether a known job already covers a turn-end seq of this session. */
function isCovered(known, sessionId, seq) {
