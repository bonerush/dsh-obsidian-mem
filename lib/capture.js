...
async function runOneJob(context, job) {
  ...
  if (binding === null || binding === undefined) {
    ...
    return { jobId: job.jobId, status: 'deferred', reason: 'no-binding', dueAt: context.now() + context.idleMs }
  }
...
async function attemptJob(context, job, binding) {
  ...
  if (outcome !== null && outcome.state === 'deferred') {
    ...
    return { jobId: job.jobId, status: 'deferred', reason: outcome.reason, dueAt: context.now() + context.idleMs }
  }
  try {
...
async function auditRefusals(context, job, outcome, barrierState) {
  ...
  // authority left.
  const rederived = barrierState.validated === null && persisted !== null && typeof persisted.raw === 'string'
...
function rederiveRefusals(context, job, raw) {
  try {
    ...
    warnLine(context.warn, `obsidian-mem: re-deriving the refusal audit of ${job.jobId} failed: ${describeError(error)}`)
    return null
  }
}
