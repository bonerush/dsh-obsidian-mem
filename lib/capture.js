...
//     (`source.kind` `plugin:<name>`, `skill-catalog`, `tool`, …) never enter;
...
import { distillCandidates, distillSettings, runPendingJob } from './distill.js'
 ...
 * @param {(reason: string) => void} [options.onSkip] - called with the reason when this
 *   returns `null`. Ten different conditions produce that one `null`, and "a turn ended
 *   and nothing was captured" is the question this seam exists to answer; the sink is
 *   advisory, so a broken one can never lose a capture.
...
export async function enqueueTurn({
  ...
  if (config?.autoCapture === false) return skip(onSkip, 'auto-capture-off')
  if (!isCompletedTurnEnd(event)) return skip(onSkip, 'not-turn-end')
  if (!isRootSession(session)) return skip(onSkip, 'not-root-session')
  const sessionId = sessionIdOf(session)
  if (sessionId === null) return skip(onSkip, 'no-session-id')
  const projectId = boundProjectId(binding)
  if (projectId === null) return skip(onSkip, 'no-project-id')
  ...
  if (endSeq === null) return skip(onSkip, 'no-end-seq')
  ...
  return withQueueLock(queueRoot, async () => {
    ...
    if (own.userMessages === 0) return skip(onSkip, 'no-user-messages')
    if (own.segments.length === 0) return skip(onSkip, 'no-segments')
    ...
    if (isProcessed(processed, endSeq)) return skip(onSkip, 'already-processed')
    ...
    const projection =
      base.fromSeq === fromSeq && base.toSeq === endSeq
        ? own
...
export async function recoverPending({ queueRoot, warn } = {}) {
  ...
  try {
    ...
    warnLine(
      warn,
      `obsidian-mem: pending recovery could not read ${queueRoot}: ${describeError(error)}`,
    )
    return {
      jobs: [],
      recovered: 0,
      invalid: [{ file: queueRoot, reason: error?.code ?? 'queue-unreadable' }],
    }
  }
  try {
...
/**
 * Return `null` from `enqueueTurn` after telling an advisory sink why.
 *
 * The sink is called through a catch because it is a diagnostic: a caller that
 * passes something broken must still get the pre-existing `null`, exactly as it
 * would have without the seam.
 *
 * @param {Function|undefined} onSkip - the caller's sink.
 * @param {string} reason - the lowercase reason token.
 * @returns {null} always `null`, so the call site reads as one statement.
 */
function skip(onSkip, reason) {
  try {
    if (typeof onSkip === 'function') onSkip(reason)
  } catch {
    /* a diagnostic never changes an outcome */
  }
  return null
}
 ...
 * @param {object} [options.diagnostics] - the decisions ring; `capture` events land here.
...
export function createCapture({
  ...
  async function captureTurn(session, event) {
    ...
    if (resolved === null) {
      recordDiagnostic(diagnostics, 'capture', { outcome: 'skipped-unbound' })
    ...
    const job = await enqueueTurn({
      ...
      now,
      onSkip: (reason) =>
        recordDiagnostic(diagnostics, 'capture', {
          outcome: reason,
          projectId: boundProjectId(resolved) ?? undefined,
        }),
    })
    ...
    try {
      ...
      warnLine(
        warn,
        `obsidian-mem: could not hand the captured job to the queue worker: ${describeError(error)}`,
      )
    }
  ...
  async function disposed(target) {
    const session =
      target !== null && typeof target === 'object' && target.session !== undefined
        ? target.session
        : target
    const id = sessionIdOf(session)
...
/**
 * Shortest worker delay. A deadline that is already past still gets one beat
 * instead of a zero-delay timer: a deferred job whose idle window is `0` must
 * never be able to turn the worker into a spin loop.
 */
export const MIN_WORKER_WAIT_MS = 100
 ...
 * @param {Function} [options.isStopped] - `() => boolean`: whether the owning worker has stopped
 *   and this pass is only being allowed to settle. Once it answers `true`, no further job is
 *   started; the rest of the due jobs are deferred as `unloaded` without consuming an attempt.
...
async function runQueuePass(options) {
  ...
  const dataRoot =
    typeof options.dataRoot === 'string' && options.dataRoot !== ''
      ? options.dataRoot
      : dirname(resolve(queueRoot))
  const settings = distillSettings(config)
  const idleMs = Number.isSafeInteger(options.debounceMs)
    ? Math.max(0, options.debounceMs)
    : idleMsOf(config)
  // A pass that is only settling must not START another job. The `llm` handle it
  // snapshotted at its own start dies with the plugin tree (measured: `NO_ADAPTER`
  // once the provider fiber is DISPOSED, `docs/p0-compatibility.md` §9), so
  // starting one would record a failure the model never produced and burn the
  // R43 bound — the very defect the stop()/abort() split exists to stop
  // inflicting on the in-flight job.
  ...
  try {
    ...
    warnLine(
      warn,
      `obsidian-mem: the pending queue at ${queueRoot} could not be read: ${describeError(error)}`,
    )
    return summary
  }
  ...
  for (const job of jobs) {
    ...
    if (isStopped !== null && isStopped() === true) {
      // Deferred, not attempted, and deliberately not written: the job keeps its
      // state, its attempt count and its due time, so the next process picks it up
      // exactly where this one left it.
      summary.deferred += 1
      results.push({ jobId: job.jobId, status: 'deferred', reason: 'unloaded' })
      continue
    }
    if (job.state === 'failed') {
      summary.failed += 1
    ...
    if (Number.isFinite(outcome.dueAt)) {
      summary.nextDueAt =
        summary.nextDueAt === null ? outcome.dueAt : Math.min(summary.nextDueAt, outcome.dueAt)
    }
...
function dueAtOf(job, idleMs) {
  ...
  if (
    job.output == null &&
    (job.attempts ?? 0) === 0 &&
    typeof job.retriedAt !== 'string' &&
    (job.state === 'pending' || job.state === 'deferred')
  ) {
...
async function runOneJob(context, job) {
  ...
  try {
    binding =
      typeof context.resolveBinding === 'function'
        ? await context.resolveBinding(job)
        : await resolveRegisteredProject({
            projectId: job.projectId,
  ...
  if (binding === null || binding === undefined) {
    await markJob(
      ...
      {
        state:
          job.output?.state === 'validated' || job.output?.state === 'raw-durable'
            ? job.output.state
            : 'deferred',
    ...
    // Same rule as a missing route: a deferral carries the deadline that makes the
    // worker come back to it on its own.
    recordDiagnostic(context.diagnostics, 'job', {
      outcome: 'deferred',
      jobId: job.jobId,
      code: 'no-binding',
      projectId: job.projectId,
    ...
    return {
      ...
      status: 'deferred',
      reason: 'no-binding',
      dueAt: context.now() + context.idleMs,
    }
  }
...
async function attemptJob(context, job, binding) {
  ...
  await markJob(
    job.jobId,
    { state: 'distilling', deferredReason: null },
    { queueRoot: context.queueRoot },
  )
  let outcome
  ...
  if (outcome !== null && outcome.state === 'deferred') {
    await markJob(
      job.jobId,
      { state: 'deferred', deferredReason: outcome.reason },
      { queueRoot: context.queueRoot },
    )
    // A missing service or route is not a failure, but it is not "done" either:
    // without a deadline the worker arms no timer and the job would wait for an
    // unrelated capture or a restart. Re-check it once per idle window.
    recordDiagnostic(context.diagnostics, 'distill', {
      outcome: 'deferred',
      jobId: job.jobId,
      code: outcome.reason,
      projectId: job.projectId,
 ...
 * `items`/`raw`/`usage`, and a resume reports `refused: []`. The audit therefore
 * gets its own barrier BEFORE anything is applied:
 *
 *   * **The fresh run** already holds the authoritative list, so it is written
 *     straight through.
 *   * **A resume** whose stored output has never been audited
 *     (`output.refused === null`) re-derives the drop set from the persisted
 *     `raw` with the same deterministic validation the first run used. That is
 *     what closes the window between the validated barrier and this write: a
 *     SIGKILL there would otherwise leave the only record of a dropped candidate
 *     nowhere, and `deleteJob` would make the loss permanent.
 *
 * The write is additive: `persistJobOutput` copies `raw`/`items`/`usage` from the
 * file on disk and can only ADD `refused`, so it can never re-mint an identity. If
 * the audit cannot be produced at all the job is failed (bounded, visible, R43b)
 * rather than completed with an unknown drop set — a receipt that says "nothing
 * was refused" when it cannot know would be the exact silence this exists to
 * prevent.
 *
 * @param {object} context - the pass context.
 * @param {object} job - the job.
 ...
 * @returns {Promise<object[]|null>} the bounded, persisted refusal audit (`[]` when nothing was refused, `null` only when there is no durable output to audit).
 * @throws {PendingError} when the stored output cannot be re-derived.
 */
async function auditRefusals(context, job, outcome, barrierState) {
  const persisted =
    barrierState.validated?.output ?? (job.output?.state === 'validated' ? job.output : null)
  ...
  // On a resume the barrier was not written in this run, so `outcome.refused` is
  // Task 15's deliberately-empty resume answer and the persisted raw is the only
  // authority left.
  const rederived =
    barrierState.validated === null && persisted !== null && typeof persisted.raw === 'string'
      ? rederiveRefusals(context, job, persisted.raw)
      : fresh
  if (persisted === null) return Array.isArray(rederived) ? rederived : null
  if (!Array.isArray(rederived)) {
    throw new PendingError(
      'audit-unavailable',
      `the per-item refusals of job ${job.jobId} could not be re-derived from its persisted output, so completing it would lose the audit`,
    )
  }
  const annotated = await persistJobOutput(
    job.jobId,
    {
      state: 'validated',
      raw: persisted.raw,
  ...
  return annotated?.output?.refused ?? rederived
}

/**
 * Re-derive one persisted output's refusal set, deterministically and offline.
 *
 * The drop set is a pure function of the raw text, the job's own evidence and the
 * project id — `distillCandidates` performs no I/O and issues no model call, and
 * the survivors' positional idempotency keys are unaffected. A refusal *throw*
 * here (for example a config bound that changed under a stored output) returns
 * `null`, which the caller treats as "cannot audit" rather than as "nothing was
 * refused".
 *
 * @param {object} context - the pass context.
 * @param {object} job - the job whose output is being audited.
 * @param {string} raw - the persisted model text.
 * @returns {object[]|null} the refused items, or `null` when it cannot be derived.
 */
function rederiveRefusals(context, job, raw) {
  try {
    const derived = distillCandidates(raw, job, context.settings)
    return Array.isArray(derived?.refused) ? derived.refused : []
  } catch (error) {
    warnLine(
      context.warn,
      `obsidian-mem: re-deriving the refusal audit of ${job.jobId} failed: ${describeError(error)}`,
    )
    return null
  }
}
...
async function applyOutcome(context, job, binding, outcome, barrierState) {
  ...
  const header = {
    ...
    // `null` means "no durable output to audit" (the receipt then says so instead
    // of claiming nothing was dropped); an array is the bounded reason list.
    refused,
    refusedCount: refused === null ? null : refused.length,
  ...
  if (header.dryRun || items.length === 0) {
    ...
    const receipt = {
      ...header,
      result: items.length === 0 ? 'no-memory' : 'dry-run',
      items: items.map(previewItem),
      index: 'none',
    }
    recordDiagnostic(context.diagnostics, 'distill', {
      ...
      ms: boundedMs(header.durationMs),
    })
  ...
  for (let position = 0; position < items.length; position += 1) {
    ...
    if (typeof context.index === 'function') {
      try {
        ...
        if (handle !== null && handle !== undefined && typeof handle.refresh === 'function')
          await handle.refresh()
        if (indexState !== 'failed') indexState = 'refreshed'
      } catch (error) {
        indexState = 'failed'
        // The write is already durable, so a failed refresh is not a failed write
        // — but it is the reason a search right after a write does not see it, and
        // that is invisible from every other surface.
        recordDiagnostic(context.diagnostics, 'index', {
        ...
        warnLine(
          context.warn,
          `obsidian-mem: refreshing the index after ${written?.path ?? item.preassignedId} failed: ${describeError(error)}`,
        )
      }
    }
    views.push({
  ...
  // `none` means this wiring had no index seam at all; `refreshed` means every
  // item followed its write. A `failed` refresh was already recorded per item,
  // above, with the code that caused it — recording it twice would push a second
  // entry into a 200-event ring for one fact.
...
async function completeJob(context, job, receipt) {
  ...
  try {
    ...
    warnLine(
      context.warn,
      `obsidian-mem: reconciling the processed floor for ${job.jobId} failed: ${describeError(error)}`,
    )
  }
...
async function failJob(context, job, error) {
  ...
  try {
    current =
      (await readPendingJobs(context.queueRoot)).jobs.find((entry) => entry.jobId === job.jobId) ??
      job
  } catch {
  ...
  if (attempts >= context.maxAttempts) {
    ...
    warnLine(
      context.warn,
      `obsidian-mem: pending job ${job.jobId} failed permanently after ${attempts} attempt(s) (${code}): ${describeError(error)}`,
    )
    recordDiagnostic(context.diagnostics, 'job', {
      outcome: 'failed',
      jobId: job.jobId,
  ...
  patch.state =
    current.output?.state === 'validated' || current.output?.state === 'raw-durable'
      ? current.output.state
      : 'pending'
  patch.nextAttemptAt = new Date(dueAt).toISOString()
  warnLine(
    context.warn,
    `obsidian-mem: pending job ${job.jobId} attempt ${attempts} failed (${code}); next attempt in ${delay} ms`,
  )
  recordDiagnostic(context.diagnostics, 'job', {
    outcome: 'retry',
    jobId: job.jobId,
...
/**
 * One duration as the diagnostics ring accepts it: a whole, non-negative number.
 *
 * The ring refuses anything else rather than rounding it, so the rounding has to
 * be a decision made here. A missing duration stays missing — `0` would claim a
 * measurement nobody took.
 *
 * @param {unknown} value - the measured duration in milliseconds.
 * @returns {number|undefined} the bounded value, or `undefined`.
 */
...
export async function retryJob(jobId, { queueRoot = defaultQueueRoot() } = {}) {
  ...
  const state =
    outputState === 'validated' || outputState === 'raw-durable' ? outputState : 'pending'
  return markJob(
    jobId,
    {
      state,
 ...
 * is running is the same pass (single-flight).
 *
 * `stop` and `abort` are two different things, and the difference is measured
 * rather than stylistic (`docs/p0-compatibility.md` §9):
 *
 *   * **`stop()` is the lifecycle seam the host's fiber disposer drives.** It
 *     stops scheduling and lets an in-flight pass settle. It deliberately does
 *     NOT abort the model call: DSH disposes the whole plugin tree the moment a
 *     headless run's session completes, and that disposal lands while the
 *     worker's call is still streaming. `ctx.llm.stream()` reports a caller-side
 *     abort as a terminal `finish.reason.kind === 'aborted'`, so aborting here
 *     records a failed attempt the model never produced, consumes the R43 bound,
 *     and is why the automatic write never completed in a real host. The stream
 *     itself survives the tree teardown (measured: a 7.2 s stream kept producing
 *     for 4.7 s after its provider fiber was DISPOSED), so letting it settle
 *     finishes the documented pipeline; every step after it is already crash-safe
 *     and item-idempotent, so a process that dies mid-apply still resumes.
 *     Settling is exactly ONE job: the pass deferred-and-not-started every job
 *     after the one already in flight (`isStopped`), because the `llm` handle the
 *     pass snapshotted at its start is dead from the disposal onwards — starting
 *     another job with it would fail that job for a reason the model never
 *     produced, i.e. the same defect one job later.
 *   * **`abort()` is a real cancellation** (a user or a test asking for the work
 *     to stop). It aborts the in-flight pass, whose terminal `aborted` is then a
 *     truthful reason for the recorded attempt.
 *
 ...
 * @returns {{start: Function, kick: Function, pass: Function, stop: Function, abort: Function, isRunning: Function}} the worker.
 */
export function createQueueWorker(options = {}) {
  ...
  /** Stop scheduling new passes; an in-flight pass keeps the signal it was given. */
  function halt() {
    stopped = true
    if (timer !== null) {
      clearTimeout(timer)
  ...
  function schedule(dueAt) {
    if (stopped) return
    // A past deadline still waits one beat: some jobs report a deadline of "now"
    // (a zero idle window, a zero backoff), and a 0 ms timer would spin.
    const delay = Math.min(Math.max(MIN_WORKER_WAIT_MS, dueAt - clock()), MAX_WORKER_WAIT_MS)
  ...
  async function pass() {
    ...
    running = processQueue({
      ...
      // The pass snapshots `llm` once, and that snapshot dies with the plugin
      // tree; once this worker has stopped, the pass may only finish the job it
      // already started, never begin another (`docs/p0-compatibility.md` §9).
      isStopped: () => stopped,
      warn,
    })
    try {
  ...
  return {
    ...
    /**
     * Cancel an in-flight pass on purpose.
     *
     * Deliberately not called by `stop()` and, today, not called by the host at
     * all (R50): the host only ever unloads the plugin, which is not a caller
     * cancellation. It is kept because a genuine cancellation must remain
     * expressible without editing this module, and because it is the only path
     * whose `aborted` reason is truthful rather than manufactured.
     */
    abort() {
      halt()
      controller.abort()
  ...
  for (const event of events) {
    ...
    if (event.type === 'assistant/message' && seq === finalSeq) {
      ...
      segments.push({
        ...
        seq,
        header: `--- assistant final (seq ${seq}) ---`,
        text: body,
      })
      continue
    }
    if (event.type === 'tool/result') {
      ...
      const callId =
        typeof part.toolCallId === 'string' ? part.toolCallId : event.data?.message?.source?.callId
      ...
      segments.push({
        kind: 'tool',
        seq,
        header: `--- tool ${name ?? 'unknown'} ${ok ? 'ok' : 'failed'} (seq ${seq}) ---`,
        text: '',
      })
    }
  }
...
function buildSafeInput({ sessionId, projectId, turn, fromSeq, toSeq, segments, maxChars }) {
  ...
  return {
    ...
    omitted:
      sorted.length === 0
        ? null
        : { seqs: sorted, fromSeq: sorted[0], toSeq: sorted[sorted.length - 1] },
  }
}
...
function eventsOf(session, from, to) {
  ...
  const snapshot =
    typeof session.snapshotEvents === 'function' ? session.snapshotEvents(from, to) : null
...
function hasToolCall(content) {
  return (
    ...
    content.some((part) => part !== null && typeof part === 'object' && part.type === 'tool-call')
  )
}
...
function toolResultPart(event) {
  ...
  const part = content.find(
    (candidate) =>
      candidate !== null && typeof candidate === 'object' && candidate.type === 'tool-result',
  )
  return part ?? null
}
...
function routeOf(session) {
  if (
    ...
    typeof session.requestContext !== 'function'
  )
    return null
  let context
  try {
