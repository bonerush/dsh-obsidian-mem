...
export function splitRelativeVaultPath(relativePath) {
  ...
  for (const segment of relativePath.split('/')) {
    ...
    if (segment === '..') throw new PathSafetyError(`path traversal is not allowed: ${relativePath}`)
 ...
 */
export async function resolveVaultFile(vaultRoot, relativePath, { mustExist = false, home = homedir() } = {}) {
  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    try {
      ...
      if (mustExist) throw new Error(`vault path does not exist: ${relativePath}`)
      break
    }
