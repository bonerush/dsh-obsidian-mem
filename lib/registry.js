 ...
 */
const REGISTRY_REGION_PATTERN =
  /<!--\s*obsidian-mem:registry begin([^>]*)-->(\r?\n)?([\s\S]*?)<!--\s*obsidian-mem:registry end\s*-->/
/** Control characters cannot survive a single-line table cell. */
// eslint-disable-next-line no-control-regex -- detecting them is how the cell stays single-line
...
export function parseRegistryMarkdown(text, { path = REGISTRY_RELATIVE_PATH } = {}) {
  ...
  if (begins !== ends)
    throw new RegistryError('registry-invalid', `${path} has an unterminated registry region`)
  if (begins > 1)
    throw new RegistryError('registry-invalid', `${path} has more than one registry region`)
  if (begins === 0) return { regionPresent: false, rows: [] }
  const match =
    /<!--\s*obsidian-mem:registry begin[^>]*-->([\s\S]*?)<!--\s*obsidian-mem:registry end\s*-->/.exec(
      body,
    )
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (cells.length !== 4) {
      throw new RegistryError(
        'registry-invalid',
        `${path} row ${index + 1} must have exactly 4 columns`,
      )
    }
    ...
    if (!isUuidV4(projectId)) {
      throw new RegistryError(
        'registry-invalid',
        `${path} row ${index + 1} has no UUIDv4 projectId`,
      )
    }
    const dir = hubCellToDir(hubCell)
    if (!dir)
      throw new RegistryError('registry-invalid', `${path} row ${index + 1} has no hub path`)
  ...
  for (const row of rows) {
    ...
    if (shape === null) {
      throw new RegistryError(
        'registry-invalid',
        `${path} hub path is not ${PROJECTS_DIR}/<slug>--<id8>: ${row.dir}`,
      )
    }
    if (shape.id8 !== row.projectId.slice(0, 8)) {
      throw new RegistryError(
        'registry-invalid',
        `${path} hub path ${row.dir} does not match ${row.projectId}`,
      )
    }
  }
...
export async function readRegistry(vaultRoot) {
  ...
  try {
    ...
    if (typeof error?.code === 'string') {
      throw new RegistryError(
        'registry-unreadable',
        `cannot read ${configuredPath}: ${error.code}`,
        { cause: error },
      )
    }
    throw error
