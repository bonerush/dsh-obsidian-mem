...
export async function resolveBinding({ cwd, vaultRoot, mode, dataRoot = null, home = homedir() } = {}) {
  ...
  try {
    ...
    if (error instanceof RegistryError) {
      return unwindAndRefuse(refusal(error.code, error.message, { repoRoot, vaultRoot: vault.root }))
    }
    throw error
  }
  ...
  if (registered !== null) {
    ...
    if (owner !== undefined && owner !== pointer.projectId) {
      return unwindAndRefuse(refusal('directory-taken', `${relativeDir} already belongs to project ${owner}`, {
  ...
  try {
    ...
    if (error instanceof PathSafetyError) {
      return unwindAndRefuse(refusal('project-directory-symlink', error.message, {
        repoRoot, vaultRoot: vault.root, relativeDir,
      }))
    }
    throw error
  ...
  if (registered !== null) {
    ...
    if (projectStat === null) {
      return unwindAndRefuse(refusal('registry-directory-missing', `${registry.path} lists ${relativeDir}, but it does not exist in the vault`, {
        repoRoot, vaultRoot: vault.root, relativeDir, projectDir,
      }))
    }
    if (!projectStat.isDirectory()) {
      return unwindAndRefuse(refusal('project-directory-not-a-directory', `${projectDir} is not a directory`, {
        repoRoot, vaultRoot: vault.root, relativeDir, projectDir,
      }))
    }
  }
...
const MOC_DIRECTORIES = Object.freeze(['Docs', 'Decisions', 'Conventions', 'Pitfalls', 'Daily', 'Inbox'])
...
function bootstrapTargets(project, today) {
  ...
  const files = [
    ...
    { path: 'Methods/index.md', contents: mocTemplate({ title: 'Methods', projectId: null, slug: null, today }) },
