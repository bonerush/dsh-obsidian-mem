 ...
 * skeleton file is created or the registry row is committed (creating the vault
 * root alone does not count — it is not project content, and any later bind
 * reuses it). A caller that minted a pointer before calling uses that flag to
 * decide whether the identity still describes real content.
 *
...
export async function bootstrapVault(binding, { initGitOnCreate = false, home = homedir(), dataRoot = resolveDataRoot() } = {}) {
  ...
  // it is set once this call creates project content or commits the registry
  // row, and attached to whatever the call throws. Creating only the vault root
  // (which is not project content and is reused by any later bind) does not set
  // it, so a template refusal after that mkdir can still be retried cleanly.
