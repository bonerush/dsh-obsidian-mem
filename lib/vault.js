...
// `_meta/registry.md`. The directory name `Projects/<slug>--<projectId 前8位>/` is a
...
// `TransactionError` must be *imported*, not only re-exported below: a
// re-export puts the name in the module's export namespace and not in its
// scope, so a `catch` that tests `instanceof TransactionError` would throw a
// ReferenceError on exactly the failure it was written to report.
import { TransactionError, newTransactionId, runTransaction } from './transaction.js'
import {
 ...
 *
 *   * `show` reports; it never creates or repairs a pointer.
 *   * omitting `mode` is the internal create path (the fixtures and the binding
 *     tests use it): for a Git root it mints or inherits exactly like `local`,
 *     while a non-Git directory still resolves `no-git-root`. Anything that must
 *     not create passes `show` explicitly — every read seam does.
...
export async function resolveBinding({
  ...
  if (isInside(cwdReal, vault.root)) {
    return {
      ...
      cwd: cwdReal,
      message:
        'the working directory is inside the memory vault; the vault itself is not a project',
    }
  }
  ...
  if (repoRoot === null) {
    return {
      ...
      vaultRoot: vault.root,
      message:
        'this directory is not in a git repository, so it stays read-only and is not added to long-term memory',
  ...
  if (mode === 'fork') {
    if (existing === null) {
      return refusal(
        'no-pointer',
        `${pointerPath} does not exist, so there is no project identity to fork away from`,
        {
          repoRoot,
          vaultRoot: vault.root,
          pointerPath,
  ...
  if (mode === 'retain') {
    if (existing === null) {
      return refusal(
        'no-pointer',
        `${pointerPath} does not exist, so there is no binding to confirm`,
        {
          repoRoot,
          vaultRoot: vault.root,
          pointerPath,
  ...
  if (existing !== null) {
    ...
    if (!pointerCreated) {
      const raced = await readPointerOrRefuse(repoRoot, {
        repoRoot,
        vaultRoot: vault.root,
        pointerPath,
      })
  ...
  /**
   * Refuse without leaving behind a pointer this call just created.
   *
   * A resolution that mints first and refuses later — an unreadable registry, a
   * directory the registry already assigns to another id, a project directory
   * that is a symlink — would otherwise leave this repository bound to an
   * identity the vault never accepted. The next attempt would read *that*
   * pointer instead of minting again, turning a one-off refusal into a sticky
   * one. Only a pointer this call created is removed (`pointerCreated`), and an
   * inherited one counts: this worktree did not hold a pointer before the call.
   *
   * @param {object} value - the refusal to return.
   * @returns {Promise<object>} the refusal, annotated with whether the pointer was removed.
   */
  async function unwindAndRefuse(value) {
    if (!pointerCreated) return value
    const unwound = await removeOwnPointer(repoRoot, pointer.projectId)
    return { ...value, unwound }
  }
  ...
  try {
    ...
    if (error instanceof RegistryError) {
      return unwindAndRefuse(
        refusal(error.code, error.message, { repoRoot, vaultRoot: vault.root }),
      )
    }
    throw error
  }
  ...
  if (registered !== null) {
    ...
    if (owner !== undefined && owner !== pointer.projectId) {
      return unwindAndRefuse(
        refusal('directory-taken', `${relativeDir} already belongs to project ${owner}`, {
          repoRoot,
          vaultRoot: vault.root,
          relativeDir,
        }),
  ...
  try {
    ...
    if (error instanceof PathSafetyError) {
      return unwindAndRefuse(
        refusal('project-directory-symlink', error.message, {
          repoRoot,
          vaultRoot: vault.root,
          relativeDir,
        }),
  ...
  if (registered !== null) {
    ...
    if (projectStat === null) {
      return unwindAndRefuse(
        refusal(
          ...
          `${registry.path} lists ${relativeDir}, but it does not exist in the vault`,
          {
            repoRoot,
            vaultRoot: vault.root,
            relativeDir,
    ...
    if (!projectStat.isDirectory()) {
      return unwindAndRefuse(
        refusal('project-directory-not-a-directory', `${projectDir} is not a directory`, {
          repoRoot,
          vaultRoot: vault.root,
          relativeDir,
          projectDir,
...
async function readPointerOrRefuse(repoRoot, context) {
  ...
  try {
    ...
    if (error instanceof PointerError)
      return { refusal: refusal(error.code, error.message, context) }
    throw error
  }
  if (raced === null) {
    return {
      refusal: refusal(
        'pointer-race',
        `${context.pointerPath} appeared and vanished while binding`,
        context,
      ),
    }
  }
...
async function forkProject({ repoRoot, vaultRoot, pointerPath, existing, home }) {
  ...
  if (next.projectId === existing.pointer.projectId) {
    return refusal(
      'fork-same-id',
      'the fresh identity collided with the current one; nothing was changed',
      { repoRoot, vaultRoot, pointerPath },
    )
  }
  ...
  if (registry.byId.has(next.projectId)) {
    return refusal(
      'fork-id-taken',
      `${next.projectId} already owns ${registry.byId.get(next.projectId)} in the vault registry`,
      { repoRoot, vaultRoot, pointerPath },
    )
  }
  const owner = registry.byDir.get(relativeDir)
  ...
  if ((await lstatOrNull(projectDir)) !== null) {
    return refusal(
      'directory-taken',
      `${relativeDir} already exists in the vault, so the fork would write into an unowned directory`,
      { repoRoot, vaultRoot, relativeDir },
    )
  }

  const swapped = await replacePointerAtomic(pointerPath, existing.bytes, next)
  if (swapped !== null)
    return refusal(swapped.reason, swapped.message, { repoRoot, vaultRoot, pointerPath })
...
async function replacePointerAtomic(pointerPath, expected, next) {
  ...
  try {
    ...
    return {
      ...
      message: `cannot re-read ${pointerPath}: ${error.code ?? error.message}`,
    }
  }
  if (!current.equals(expected)) {
    return {
      reason: 'pointer-changed',
      message: `${pointerPath} changed after it was read; refusing to replace another writer's identity`,
    }
  }
  ...
  try {
    ...
    return {
      ...
      message: `cannot replace ${pointerPath}: ${error.code ?? error.message}`,
    }
  }
  let written
  try {
    ...
    return {
      reason: 'pointer-write-failed',
      message: `${pointerPath} is unreadable after the fork swap: ${error.code ?? error.message}`,
    }
  }
  if (written.projectId !== next.projectId) {
    return {
      reason: 'pointer-write-failed',
      message: `${pointerPath} does not carry the new identity after the fork swap`,
    }
  }
  return null
}
...
async function retainProject({ repoRoot, vaultRoot, pointerPath, existing, dataRoot, home }) {
  if (typeof dataRoot !== 'string' || dataRoot.trim() === '') {
    throw new RangeError(
      'mode "retain" rewrites the vault registry and therefore needs the plugin data root',
    )
  }
  ...
  if (remote === null) {
    return refusal(
      'no-remote',
      'this repository has no origin remote, so there is no hint to confirm',
      { repoRoot, vaultRoot, pointerPath },
    )
  }
  let registry
  ...
  if (row === undefined) {
    return refusal(
      'unregistered',
      `${REGISTRY_RELATIVE_PATH} has no row for ${projectId}; bootstrap the project before confirming its remote`,
      { repoRoot, vaultRoot, pointerPath },
    )
  }
  const binding = {
...
function retainRemoteTransform(text, projectId, remote) {
  ...
  if (region === null) {
    throw new BootstrapError(
      'registry-invalid',
      `${REGISTRY_RELATIVE_PATH} has no generated region to update`,
    )
  }
  if (region.declaredHash === null) {
    throw new BootstrapError(
      'registry-hash-missing',
      `${REGISTRY_RELATIVE_PATH} has no sha256 hash in its registry marker`,
    )
  }
  if (!region.matches) {
    throw new BootstrapError(
      'registry-hash-mismatch',
      `${REGISTRY_RELATIVE_PATH} does not match its recorded sha256; refusing to write it`,
    )
  }
  ...
  if (own === undefined) {
    throw new BootstrapError(
      ...
      `${REGISTRY_RELATIVE_PATH} no longer lists ${projectId}`,
    )
  }
  if (own.remote === remote) return null
  const nextRows = rows.map((entry) =>
    entry.projectId === projectId ? { ...entry, remote } : entry,
  )
...
async function inheritSiblingPointer(repoRoot) {
  ...
  if (scan.unreadable.length > 0) {
    return {
      failure: refusal(
        'sibling-unreadable',
        'a sibling worktree could not be read, so no new project id is invented',
        {
    ...
    if (!samePointer(first, other)) {
      return {
        failure: refusal(
          ...
          'sibling worktrees carry different .obsidian-mem pointers; bind explicitly',
          {
            details: [first, other],
          },
        ),
...
async function detectConcurrentBind(repoRoot, projectId) {
  ...
  if (!scan.ok) {
    return refusal(
      'git-unavailable',
      `cannot re-check sibling worktrees after writing ${POINTER_FILENAME}: ${scan.message}`,
    )
  }
  ...
  if (scan.unreadable.length > 0) {
    return refusal(
      'sibling-unreadable',
      'a sibling worktree could not be re-read after writing the pointer',
      {
        details: scan.unreadable,
      },
    )
...
/**
 * Undo the pointer a binding created, when that binding created one.
 *
 * The counterpart of a refused automatic first write (spec §5.2.3/§5.3): a
 * bootstrap that refuses before its first vault write — the property preflight
 * of §6.4/R18 — must leave the repository exactly as the write found it. If the
 * pointer this bind minted stayed on disk, the *next* write would resolve as
 * bound and skip the preflight that just refused it. A pointer this call did
 * not create (an existing one, or one another writer won the race with) is never
 * touched: `pointerCreated` is the only signal that acts, and `removeOwnPointer`
 * re-reads the file so a concurrent writer's identity survives.
 *
 * @param {object} binding - a `kind:'bound'` resolution.
 * @returns {Promise<boolean>} whether a pointer was removed.
 */
export async function releaseCreatedPointer(binding) {
  if (binding === null || typeof binding !== 'object' || binding.kind !== 'bound') return false
  if (binding.pointerCreated !== true) return false
  return removeOwnPointer(binding.repoRoot, binding.projectId)
}
...
const MOC_DIRECTORIES = Object.freeze([
  'Docs',
  'Decisions',
  'Conventions',
  'Pitfalls',
 ...
 *
 * That registry update is a *transaction* (Task 7): `_meta/registry.md` is shared
 ...
 * (`template-invalid`). The registry step refuses an existing region whose
 * recorded sha256 does not cover its body (`registry-hash-mismatch`) for the same
 * reason: it is read before anything is written.
 *
 * Every error this call throws carries `vaultWritten`, which is `false` until a
 * skeleton file **or directory** is created or the registry row is committed
 * (creating the vault root alone does not count — it is not project content, and
 * any later bind reuses it; the `targets` loop sets the flag for a created
 * directory too, which is the conservative side: it keeps an identity whose
 * project directory already exists on disk). A caller that minted a pointer
 * before calling uses that flag to decide whether the identity still describes
 * real content.
 *
...
export async function bootstrapVault(
  binding,
  { initGitOnCreate = false, home = homedir(), dataRoot = resolveDataRoot() } = {},
) {
  if (typeof initGitOnCreate !== 'boolean')
    throw new RangeError('initGitOnCreate must be a boolean')
  const project = validateBootstrapBinding(binding)
  const vault = await resolveVaultRoot(binding.vaultRoot, { home })
  if (await isCloudManagedVaultRoot(vault.root, { home })) {
    throw new BootstrapError(
      'vault-cloud-managed',
      `refusing to bootstrap the cloud-managed vault at ${vault.root}`,
    )
  }
  ...
  if (vault.exists) {
    ...
    if (preflight.conflicts.length > 0) {
      ...
      const code =
        reasons.size === 1 && reasons.has('type-conflict')
  ...
  // From here on this call can write. `vaultWritten` is the boundary a caller
  // needs to decide whether an identity it minted still describes real content:
  // it is set once this call creates project content — a skeleton file *or* a
  // skeleton directory — or commits the registry row, and attached to whatever
  // the call throws. Creating only the vault root (which is not project content
  // and is reused by any later bind) does not set it, so a template refusal after
  // that mkdir can still be retried cleanly.
  ...
  try {
    if (vault.exists) {
      ...
      if (!stat.isDirectory())
        throw new BootstrapError('vault-root-not-directory', `${vault.root} is not a directory`)
    } else {
      await fs.mkdir(vault.root, { recursive: true })
      vaultCreated = true
    }
    gitInitialized = vaultCreated && initGitOnCreate ? await initVaultGit(vault.root) : false
    ...
    if (registry.changed) {
      // Task 7: `_meta/registry.md` is shared by every project in the vault, so
      ...
      await runTransaction(
        ...
        {
          txId: newTransactionId(),
          creates: registry.created
            ? [{ path: registry.relativePath, contents: registry.nextText }]
            : [],
    ...
    if (error !== null && typeof error === 'object') error.vaultWritten = vaultWritten
    throw error
  }
 ...
 * satisfy R4 (`Projects/<slug>--<projectId 前8位>`), which is what keeps the registry
...
function validateBootstrapBinding(binding) {
  ...
  if (binding.kind !== undefined && binding.kind !== 'bound') {
    throw new BootstrapError(
      'binding-invalid',
      `bootstrapVault requires a bound binding, not ${JSON.stringify(binding.kind)}`,
    )
  }
  if (!isUuidV4(binding.projectId))
    throw new BootstrapError('binding-invalid', 'binding.projectId must be a UUIDv4')
  if (!isValidSlug(binding.slug))
    throw new BootstrapError('binding-invalid', 'binding.slug must be a vault slug')
  ...
  if (
    ...
    throw new BootstrapError(
      'binding-invalid',
      'binding.displayName must be a non-blank, control-character-free name of at most 200 characters',
    )
  }
  ...
  const relativeDir =
    binding.relativeDir === undefined || binding.relativeDir === null
  ...
  if (shape === null) {
    throw new BootstrapError(
      'binding-invalid',
      `binding.relativeDir must be ${PROJECTS_DIR}/<slug>--<projectId 前8位>: ${relativeDir}`,
    )
  }
  if (shape.id8 !== binding.projectId.slice(0, 8)) {
    throw new BootstrapError(
      'binding-invalid',
      `binding.relativeDir ${relativeDir} does not carry project id ${binding.projectId}`,
    )
  }
...
function bootstrapTargets(project, today) {
  ...
  const files = [
    ...
    { path: USER_RELATIVE_PATH, contents: userTemplate() },
    {
      ...
      contents: mocTemplate({ title: 'Methods', projectId: null, slug: null, today }),
    },
    {
      path: `${relativeDir}/index.md`,
      contents: mocTemplate({ title: displayName, projectId, slug, today, relativeDir }),
    },
...
async function prepareRegistry(vaultRoot, entry, { home }) {
  ...
  if (!stat.isFile()) {
    throw new BootstrapError(
      'registry-not-a-file',
      `${relativePath} exists in the vault but is not a regular file`,
    )
  }
  let bytes
  try {
    ...
    throw new BootstrapError(
      'registry-unreadable',
      `cannot read ${relativePath}: ${error.code ?? error.message}`,
      { cause: error },
    )
  }
 ...
 *
 * This is the whole read-modify-write of `_meta/registry.md` as a pure
...
function registryNextText(text, entry, relativePath = REGISTRY_RELATIVE_PATH) {
  ...
  if (own !== undefined && own.dir !== entry.dir) {
    throw new BootstrapError(
      ...
      `${relativePath} already maps ${entry.projectId} to ${own.dir}, not ${entry.dir}`,
    )
  }
  const owner = rows.find((row) => row.dir === entry.dir)
  if (owner !== undefined && owner.projectId !== entry.projectId) {
    throw new BootstrapError(
      'registry-directory-taken',
      `${entry.dir} already belongs to ${owner.projectId}`,
    )
  }
  ...
  if (regionPresent) {
    if (region === null) {
      throw new BootstrapError(
        'registry-invalid',
        `${relativePath} has a registry region that cannot be located`,
      )
    }
    if (region.declaredHash === null) {
      throw new BootstrapError(
        'registry-hash-missing',
        `${relativePath} has no sha256 hash in its registry marker`,
      )
    }
    if (!region.matches) {
      throw new BootstrapError(
        'registry-hash-mismatch',
        `${relativePath} generated region does not match its recorded sha256; refusing to write it`,
      )
    }
  }
  ...
  const nextText =
    region === null
      ? appendRegistryRegion(text, nextRegion)
      : spliceRegistryRegion(text, region, nextRegion)
  assertRegistryDocument(nextText, relativePath)
  return nextText
}
...
function assertRegistryDocument(text, relativePath) {
  ...
  if (!regionPresent || region === null || !region.matches) {
    throw new BootstrapError(
      'registry-invalid',
      `${relativePath} was not rendered as a valid generated region`,
    )
  }
  return rows
}
...
async function ensureDirectory(path, relativePath) {
  ...
  if (stat !== null) {
    if (stat.isSymbolicLink() || !stat.isDirectory()) {
      throw new BootstrapError(
        'not-a-directory',
        `${relativePath} exists in the vault but is not a directory`,
      )
    }
    return false
  }
