 ...
 * preflight (`property-type-conflict`) reads the vault's visible frontmatter
 * types: Obsidian registers a property's type vault-wide by name, so a vault
 * that already holds `tags: foo` must not receive `tags: […]`. And every note
 * template this call is about to create is rendered and then verified against
 * the same closed vocabulary (`template-invalid`).
 *
